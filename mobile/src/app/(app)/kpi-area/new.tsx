 ...
 *
 * "Klik Pakai Template membuka bottom sheet" → list `kpi_area_templates` di bawah goal_template_id
 * parent Goal, dikelompokkan per `division_label`. Pilih → prefill `name`.
 *
 * V1 limitation: schema `kpi_area_templates` hanya punya `name` + `division`. Target & Ekspektasi
 * Hasil rekomendasi belum ada di schema, jadi user tetap mengetik manual. Bisa di-extend
 * follow-up via ALTER kpi_area_templates ADD COLUMN target_hint/expected_outcome_hint.
 */
function KpiAreaTemplatePicker({
  goalTemplateId,
  onPick,
}: {
  goalTemplateId: string | null | undefined;
  onPick: (t: KpiAreaTemplate) => void;
}) {
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, KpiAreaTemplate[]>();
