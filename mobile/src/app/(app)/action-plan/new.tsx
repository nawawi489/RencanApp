...
export function LiveNewActionPlanScreen() {
  ...
  // Fase 4 (initiativeId) / Fase 6 (problemStatementId) — ActionPlan ditautkan ke salah satu induk
  // (mutually exclusive via CHECK action_plans_single_parent di DB). Tanpa param → ActionPlan datar.
  ...
  function submit() {
    if (!name.trim()) {
      Alert.alert('Belum lengkap', 'Nama ActionPlan wajib diisi.');
      return;
    }
  ...
  return (
    <ScrollView className="flex-1 bg-neutral-50 dark:bg-black" keyboardShouldPersistTaps="handled">
      <View className="gap-4 p-5">
        <GuidanceNote
          title="ActionPlan — Program eksekusi"
          body="ActionPlan adalah program konkret untuk menjalankan strategi. Isi Target Hasil, lalu pecah jadi Action Plan. Card disimpan sebagai Draft dulu; aktifkan setelah kelengkapan terpenuhi."
        />

        <SectionCard>
          <LabeledInput label="Nama ActionPlan" value={name} onChangeText={setName} required placeholder="mis. Kampanye Konten Q3" />
          <LabeledInput
            label="Target Hasil"
            value={target}
