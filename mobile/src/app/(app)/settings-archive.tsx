...
import { getArchiveMetadataBatch } from '@/lib/activity-governance';
import { ENTITY_ROUTE_SEGMENT } from '@/lib/entity-routes';
import { alertFriendlyError } from '@/lib/errors';
import { restoreCard, type CardEntityType, type SearchResult } from '@/lib/governance-admin';
...
export default function SettingsArchiveScreen() {
  ...
  // UI-S-AR1 metadata — ambil archived_at (action=card_archived) utk semua baris arsip dalam
  // SATU query batch, alih-alih satu round-trip per baris (dulu useQueries → ~N request paralel).
  // Peta di-key `entity_type:entity_id`; enabled hanya saat ada baris. `metaKey` stabil selama
  // kumpulan baris sama → query tak refetch tiap render/ketikan.
  const metaKey = useMemo(
    () => archived.map((r) => `${r.entity_type}:${r.id}`),
    [archived],
  );
  const { data: archiveMeta } = useQuery({
    queryKey: ['archive-metadata-batch', metaKey],
    queryFn: () =>
      getArchiveMetadataBatch(archived.map((r) => ({ entityType: r.entity_type, entityId: r.id }))),
    enabled: archived.length > 0,
  });

  const renderItem = useCallback(
    ({ item: r }: { item: SearchResult }) => {
      const meta = archiveMeta?.get(`${r.entity_type}:${r.id}`) ?? null;
    ...
    [archiveMeta, restoreM, router],
  );

  // Header (judul + note + search + filter) sebagai ListHeaderComponent — pola sama people.tsx.
  const header = (
  ...
  return (
    <View className="flex-1 bg-neutral-50 dark:bg-black">
      ...
      <FlatList<SearchResult>
        data={!enabled || isLoading ? [] : archived}
