...
| `AckCheckbox` (konfirmasi aksi ireversibel) | Checkbox pernyataan-paham di **body** dialog destruktif, menggantikan pola "kalimat panjang di label tombol" (label >302px wrap 2 baris di viewport 390 — terukur). `Pressable` `min-h-[44px] flex-row items-start gap-3 rounded-xl border px-3 py-2`; unchecked `border-neutral-300 dark:border-neutral-700`, checked `border-brand-dark bg-brand-50 dark:bg-blue-950/30`. **Glyph wajib** `✓` (checked) / `○` (unchecked) `text-base font-semibold text-brand-dark` — dua sinyal non-warna beriringan dengan border (§4 rule 2: warna ≠ satu-satunya sinyal). Teks pernyataan `flex-1 text-sm` **boleh wrap** (memang untuk dibaca, bukan label tombol). **A11y mengikat**: `accessibilityRole='checkbox'` + `accessibilityState={{ checked }}` + `accessibilityLabel` = kalimat pernyataan penuh. **Kontrak pemakaian**: tombol aksi destruktif WAJIB `disabled` sampai `checked` — dua tindakan sadar terpisah; state di-reset saat dialog ditutup supaya tidak "lengket" saat dibuka ulang. Preseden pola checkbox: `settings-card-completion-rule.tsx` | `mobile/src/components/finalize-period-modal.tsx` |
