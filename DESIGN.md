...
- **State pilihan/checked = ikon Ionicons, bukan glyph teks** (resolusi konflik §7↔§10, owner 2026-08-04): kontrol seperti `AckCheckbox` memakai `checkmark-circle` (terpilih) / `ellipse-outline` (tak terpilih) dari Ionicons — genuine perbedaan bentuk (memenuhi §4 "warna ≠ satu-satunya sinyal") **dan** Ionicons (memenuhi aturan library di sini). **Jangan** pakai glyph teks telanjang `✓`/`○`. Kode yang masih memakai glyph teks (`ui.tsx` `AckCheckbox`, `finalize-period-modal.tsx`) ditukar ke Ionicons pada pass `colorize` terpisah — bukan bagian dari kontrak dokumen ini, tapi kontraknya mengikat kode baru.
- **Warna ikon di prop non-className (`Ionicons` `color`)**: pakai hook `useThemedIcon(light, dark)` (`mobile/src/providers/theme-provider.tsx`) — bukan ternary `effective === 'dark' ? ... : ...` inline berulang. Pasangan hex yang sudah dipakai berkali-kali (daftarkan pasangan baru di sini sebelum dipakai di kode baru):
  ...
  - `warn` (ikon peringatan in-dialog, mis. `WarningCallout` / `finalize-period-modal`): `#b45309` (terang) / `#fbbf24` (gelap) — selaras status `warn` §2. Verifikasi: [`finalize-period-modal.tsx`](mobile/src/components/finalize-period-modal.tsx) `warningIconColor`.
