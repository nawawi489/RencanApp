...
3. **Policy replay bentrok pada forward re-apply** — kalau `0046` forward di-replay setelah drill (skenario tidak-realistis di produksi tapi umum di dev), section S2 melakukan `DROP POLICY IF EXISTS <old-name>` (yang sudah tidak ada) lalu `CREATE POLICY <new-name>` yang collide dgn policy new-name yg tersisa dari initial apply. **Follow-up F6a**: tambahkan `DROP POLICY IF EXISTS <new-name> ON <table>` sebelum setiap `CREATE POLICY` di generator `gen_policy_rewrites.js` supaya replay-safe. Tidak block produksi karena flow produksi = forward-satu-arah + restore snapshot via R2.
4. **Post-drill state**: local DB kembali ke state penuh V1.8.3 (4 tabel new-name, `action_plans_insert`/`tasks_select`/`strategies_select` policies aktif, `map_legacy_entity_type('kpi_area')` = `'strategy'`). tsc + jest tetap hijau (jest 1163/1163 stabil).
