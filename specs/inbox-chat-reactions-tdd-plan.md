...
- ~~Token "Reaction pill" di `DESIGN.md §7`~~ — **SELESAI 2026-07-13**: 3 entri terdaftar (`ReactionPill`/`ReactionPillRow`/`ReactionErrorAlert`); `mobile/src/global.css` tidak butuh perubahan (`brand-dark #1564b3` sudah ada + Tailwind palette cukup). Fase D dapat dieksekusi tanpa prasyarat design tambahan.
...
### Fase D — UI layer (Jest RNTL)

10. **Prasyarat token — SELESAI 2026-07-13.** `DESIGN.md §7` sudah memuat spesifikasi tiga sub-komponen: `ReactionPill` (dimensi inline numeric, unselected vs selected, sinyal non-warna border-2+✓, copy accessibilityLabel exact), `ReactionPillRow` (container + return null saat kosong + urutan konstanta `REACTION_EMOJI_ORDER`), `ReactionErrorAlert` (inline non-blocking + wajib clear di path sukses). `mobile/src/global.css` tidak berubah. Green-UI (§4 step 12) merefer entri-entri itu verbatim; UI test (§8.3 + §12.6) mengunci copy accessibilityLabel yang sama.
...
- ~~UI menyentuh pill sebelum token turun~~ **(SELESAI 2026-07-13)**: token sudah terdaftar di `DESIGN.md §7`; Fase D kini tergantung hanya pada penjadwalan V2.
...
- Token "Reaction pill" tercatat di `DESIGN.md §7` (SELESAI 2026-07-13 — 3 entri).
...
| 10 | refactor | Fase D/Prasyarat — **SELESAI 2026-07-13** (`bfa65a6`+lanjutan): DESIGN.md §7 memuat 3 entri `ReactionPill` / `ReactionPillRow` / `ReactionErrorAlert`. `mobile/src/global.css` tidak berubah (brand-dark existing + Tailwind palette cukup). Langkah ini bukan blocker lagi. | `DESIGN.md` (`mobile/src/global.css` tanpa perubahan) | FR-RX-0.3 RESOLVED, FR-RX-4.4 RESOLVED |
...
- **Ketika V2 dijadwalkan:** semua 35 item critic SUDAH diadjudikasi (§12) — eksekusi mengikuti §4 + §8 (yang telah dikoreksi in-place) + §12.3–12.6 (case tambahan). Token DESIGN sudah turun 2026-07-13 (`DESIGN.md §7` 3 entri). **Sisa prasyarat**: hanya nomor migrasi reconcile ke `HEAD_max+1` saat build (bukan keputusan spec — murni operasional).
