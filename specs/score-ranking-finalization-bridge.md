...
step1 (confirm)                 ← body menampilkan pratinjau + warning override + kalimat ireversibilitas
  ├─ dismiss (aman) → close
  ↓ user centang AckCheckbox, lalu tap "Finalisasi Periode"
...
| Modal step 1 confirm | **"Finalisasi Periode"** — **disabled sampai checkbox dicentang**. Sengaja lebih ringkas dari tombol pemicu: konteks "peringkat" sudah dibawa judul modal + paragraf body, dan label pendek menjaga satu baris di layar sempit |
...
**AC-FIN-4** *(amandemen 2026-07-20)* — Step 1 mensyaratkan **dua tindakan sadar**: centang `AckCheckbox` "Saya paham periode ini tidak dapat dibuka kembali." lalu tap **"Finalisasi Periode"** (label modal sengaja lebih ringkas dari tombol pemicu screen). Tombol **disabled** selama checkbox belum dicentang (menekan tombol terkunci TIDAK memicu RPC apa pun). Setelah keduanya → state `calculating`; label "Langkah 1 dari 2 · Menghitung skor pengguna…"; on success → state `locking`.
...
- **T-M-4** Confirm step 1 (centang `AckCheckbox` → tap "Finalisasi Periode") → state `calculating`; label "Langkah 1 dari 2 · Menghitung skor pengguna…"
