...
## [2026-07-23] update | BL-10d selesai — scope audit (migrasi 0088). PRD §38 LENGKAP 14/14

- Files created: `supabase/migrations/0088_search_global_admin.sql`, `supabase/tests/0088_search_global_admin_contract.sql`
- Files updated: `search.tsx` (label varian FR-10 + komentar 9→14 scope), test layar (+3 kasus), `specs/bl-10-search-scope-38.md` (§6.3 + §6.3.1 baru), [[feature-gap-backlog]]
- **BL10-OQ-05 terjawab lewat data, bukan preferensi.** Investigasi DB nyata: `activity_logs` 733 baris tapi hanya **11 `action` unik**, `create` sendiri 536 (**73%**); `governance_violations` 3 baris. Mencocokkan pada `action` berarti "dibuat" → nol (snake_case), "create" → tiga perempat seluruh log. **Owner memutuskan: cocokkan pada NAMA ENTITAS INDUK.**
- Peta label Indonesia di SQL **ditolak** — ia menduplikasi `GOVERNANCE_VIOLATION_TYPE_LABEL`, dan gate CI BL-13 dipasang persis untuk mencegah peta itu menyimpang. Label tetap dipetakan di klien.
- **Entitas diresolusi lewat IDENTITAS, bukan label.** `entity_type` ambigu untuk gating — `strategy` bisa berarti tabel `strategies` (baru) atau `initiatives` (warisan), gate-nya beda. Berbeda dari komentar (§6.4) yang aman karena `action_plan`/`task` sama-sama digate `can_access_task`. Diverifikasi nol tabrakan UUID lintas 7 tabel, lalu dipakai 7 `LEFT JOIN` + `CASE` gate.
- **Efek yang diinginkan: FR-11 fail-closed jadi STRUKTURAL** — baris menunjuk `period_snapshot`/tabel non-card/`entity_id` NULL gugur lewat `else false` di CASE, bukan lewat filter terpisah yang bisa lupa dipasang. Dikunci `DB-102`.
- **Tabrakan tiga keputusan yang tidak terlihat saat masing-masing diambil**: FR-10 (self-row ikut RLS) × OQ-05 (tampilkan nama entitas) × aktor tak berhak atas entitasnya = kebocoran nama. Search memilih **tidak bocor**; gate entitas berlaku juga di cabang self-row. Konsekuensi jujur: **Search bukan pengganti `/settings-activity-log`**, yang tidak diubah. Dikunci `DB-103` **berpasangan** `DB-104` (self-row tetap hidup saat entitasnya boleh dilihat) — tanpa pasangannya, DB-103 hijau pada implementasi yang keliru mematikan seluruh cabang self-row.
...
## [2026-07-23] update | BL-16 — cursor separuh di `search_global` ditolak (PR #165, migrasi 0089)

- Pages updated: `concepts/feature-gap-backlog.md` (baris BL-16 dicoret + DONE, catatan triage §2 dimutakhirkan jadi dua item tersisa, plus §BL-16)
- **Catatan urutan.** Pekerjaan ini dimulai dari `origin/staging` yang belum memuat #164, sehingga baris BL-16 belum ada saat itu (daftar berhenti di BL-15) dan sempat ditulis sebagai baris baru. #164 mendarat lebih dulu; saat rebase, baris versi #164 diganti versi selesai ini dan BL-17/BL-18 dipertahankan utuh. Isi keduanya sepakat — termasuk koreksi severity yang sama-sama dicatat.
- Files created: `supabase/migrations/0089_search_global_cursor_guard.sql`, `supabase/tests/0089_search_global_cursor_guard_contract.sql`. Nol perubahan klien.
- **Gap.** Guard bentuk-request FR-19 (0085) hanya menuntut `p_scopes` berisi tepat satu scope; ia tidak menuntut kedua bagian cursor ada. `p_cursor_ts` terisi + `p_cursor_id` NULL lolos ke keempat belas cabang, yang semuanya memfilter `(created_at, id) < (p_cursor_ts, p_cursor_id)`.
- **Kenapa hampir tak terlihat.** Postgres membandingkan tuple **short-circuit**: selama `created_at < p_cursor_ts` tegas, `id` tak pernah disentuh. Komponen kedua baru disentuh saat `created_at` persis **tie** dengan cursor — satu-satunya kondisi `id` ada untuk memecahkannya. Di sana `id < NULL` → NULL dan seluruh baris tie gugur **tanpa error**.
- **Severity dikoreksi ke bawah.** Kritik §9.1 Missing #4 `specs/bl-10-pr1-tdd-plan.md` menyebut "semua baris hilang" — **berlebihan**. Cakupan sebenarnya hanya kasus tie, dan klien tak pernah menghasilkannya (`useSearchScopePage` menyimpan cursor sebagai objek `{ts,id}` dari dua kolom non-null). Hanya terjangkau lewat panggilan API tangan.
- **Kenapa tetap dikerjakan** bukan jangkauannya melainkan **bentuk kegagalannya**: hasil hilang tanpa exception, tanpa baris nol yang mencurigakan (halaman berikutnya memang wajar kosong), tanpa jejak log. Kelas bug yang tidak akan pernah dilaporkan. Ongkos perbaikan empat baris.
- **Bentuk kebalikannya ikut ditutup**: `p_cursor_id` terisi + `ts` NULL hari ini diabaikan diam-diam oleh gate `p_cursor_ts is null` di tiap cabang — pemanggil menerima halaman pertama lagi sambil mengira ia meminta halaman berikutnya. Sama-sama salah senyap.
- **Bentuk guard**: `(p_cursor_ts is null) <> (p_cursor_id is null)` → `raise` errcode `22023`, pesan **statis** (FR-13: menyisipkan nilai baris/aktor menjadikannya oracle; `DB-98` memeriksanya). Ditaruh di **kepala fungsi**, bukan per cabang, agar cabang ke-15 ikut terlindungi. `create or replace` — `drop … cascade` akan mereset ACL ke `PUBLIC EXECUTE` dan membatalkan revoke 0085, jadi badan disalin verbatim dari 0088 dan blok `revoke`+`grant` diulang.
...
- Nomor PR: **#166**.
## [2026-07-23] update | BL-18 — scoping: penghitung FR-34 tidak pernah keluar dari perangkat

- Files updated: `mobile/src/lib/search.ts` (komentar saja, nol perubahan perilaku)
- Pages updated: [[feature-gap-backlog]] (baris BL-18 didiagnosis ulang + §BL-18 baru: 4 opsi, rekomendasi, dan apa yang harus diputuskan owner)
- **Diagnosis awal kurang satu tingkat.** Baris BL-18 berbunyi "tidak ada ambang/alerting/pembaca". Yang sebenarnya: peristiwa FR-34 ditulis pada level `info`, dan satu-satunya transport terpusat (`createSentryTransport`) **hanya meneruskan `error`+`warn`** — perilaku itu dikunci tesnya sendiri (`sentry-logger.test.ts`: *"info/debug tidak mengirim ke Sentry (hanya console)"*). Jadi penghitung per-aktor berakhir di `console.log` perangkat pemakai. Bukan "tidak ada yang membaca" — **tidak ada yang bisa**.
- **Fakta kedua yang lebih menentukan.** `search_global` di-`grant execute … to authenticated`, jadi pemegang sesi mana pun bisa memanggilnya langsung lewat PostgREST tanpa menjalankan kode klien. Telemetri yang diemisikan klien tidak dapat menjadi kontrol terhadap klien — penambang data tinggal tidak memakai aplikasinya.
- **Karena itu urutan kerja di baris BL-18 terbalik.** "Tentukan ambang wajar" lebih dulu akan menghasilkan angka rapi di atas mekanisme yang tidak menegakkan apa pun — bentuk kegagalan yang sama dengan yang dikeluhkan BL-18, satu lapis lebih dalam. Ambang baru bermakna setelah ada mekanisme yang mengikat DAN sink yang benar-benar dibaca.
- **Empat opsi ditulis dengan ongkos + sentuhan G6-nya** (§BL-18). Rekomendasi: **opsi 3** — baca jejak yang sudah dicatat Postgres/Supabase di luar aplikasi (per `jwt.sub`), ambang + alert di sisi platform. Ia mengikat karena mengamati DB, tempat setiap pemanggil harus lewat, dan **nol sentuhan G6** karena tidak menambah emisi baru. Bentuknya sejenis opsi 3 BL-13: begitu dibingkai ulang dari "apa yang harus kita tulis" jadi "apa yang sudah tercatat dan belum dibaca", ongkosnya turun dari migrasi jadi konfigurasi.
- **Opsi 1 ditolak** (naikkan level ke `warn` + ambang klien): membeli rasa aman tanpa mengikat pemanggil langsung, sekaligus membanjiri Sentry satu peristiwa per pencarian. **Opsi 2 ditolak untuk sekarang** (tabel penghitung + wrapper `volatile`): mengikat, tapi membalik keputusan G6 demi risiko yang jangkauannya belum diukur — kalau diminta owner, itu spec tersendiri.
- **Opsi 4 dikerjakan sekarang** karena benar di bawah semua opsi lain: komentar `search.ts` menyebut penghitung ini "kontrol kompensasi atas nol-emisi audit", klaim yang tidak dipenuhi implementasinya. Klaim palsu semacam itu lebih berbahaya daripada tidak ada komentar — ia membuat kontrol tercentang di review tanpa ada yang memeriksa apakah benda itu bekerja. Komentar kini menyatakan status sebenarnya + kedua faktanya.
- **Tidak ada test baru, dan itu disengaja.** Fakta "info tidak sampai Sentry" sudah dikunci `sentry-logger.test.ts`. Mengunci level `info` pada peristiwa FR-34 justru akan memprasangkai keputusan yang sedang ditunggu.
- Verifikasi: `npm test` 137 suite / 1718 test hijau, `tsc --noEmit` bersih, lint 0 error, rename-guard clean.

## [2026-07-23] update | BL-18 — keputusan owner: opsi 3, runbook `edge_logs` per aktor

- Pages created: [[search-mining-monitor]] (runbook)
- Pages updated: [[feature-gap-backlog]] (§BL-18 + baris BL-18), [[index]]
- Files updated: `mobile/src/lib/search.ts` (komentar saja)
- **Keputusan owner 2026-07-23: opsi 3** — kontrol dibangun di sisi platform, membaca jejak yang sudah dicatat, bukan menulis emisi baru. Nol sentuhan G6.
- **Kelayakannya diverifikasi, bukan diasumsikan.** Rekomendasi "baca yang sudah dicatat Postgres" tidak boleh berhenti pada keyakinan bahwa DB mencatat sesuatu; yang menentukan adalah apakah jejaknya **dapat diatribusikan ke aktor**. Hasil pemeriksaan terhadap proyek staging:
  - **`pg_stat_statements` GUGUR** meski terpasang (v1.11): ia mengagregasi per `(userid, dbid, queryid)`, dan seluruh request PostgREST terautentikasi berjalan sebagai satu role Postgres yang sama (`authenticated`). Semua aktor melebur jadi satu baris. Kalau opsi 3 dikirim di atasnya, ia akan tampak bekerja sambil tidak pernah bisa menunjuk siapa pun.
  - **`edge_logs` MEMENUHI SYARAT**: membawa `metadata.request.sb.auth_user` = `sub` JWT = `auth.users.id`, kunci yang sama dengan `actorId` FR-34 dan dengan RLS. **Diisi gateway, bukan klien** — itu properti yang membuat opsi 3 mengikat sementara opsi 1 tidak: pemanggil yang mem-bypass aplikasi lewat PostgREST tetap tercatat.
  - `pgaudit` tersedia tapi tidak terpasang, dan tidak menolong: ia mencatat pada level sesi Postgres, yang sekali lagi `authenticated` untuk semua orang.
- **Batas yang jujur dan ditulis di runbook-nya sendiri:** kueri Log Explorer diturunkan dari pola resmi `edge_logs` Supabase, **belum pernah dijalankan terhadap proyek ini**. Sampel 24 jam log `api` staging hanya memuat cron `claim_push_deliveries` — nol panggilan `search_global` — jadi tidak ada yang bisa dikonfirmasi dari sana. Kueri yang mengembalikan nol karena salah nama field terlihat persis sama dengan "tidak ada penambangan", jadi langkah pertama pemegang runbook adalah menjalankannya sekali di lingkungan yang benar-benar dipakai.
- **Ambang tetap tidak ditentukan, dan itu disengaja.** Ia butuh baseline ≥2 minggu dari trafik nyata; staging nol trafik pencarian. Prosedurnya ditulis (persentil, bukan rata-rata; ambang di atas p99 pemakaian sah — ambang yang memicu tiap minggu akan diabaikan dalam sebulan, yaitu cacat BL-18 lagi). Retensi `edge_logs` bergantung paket Supabase dan membatasi jendela baseline; harus diperiksa sebelum dijanjikan.
- **Sisa yang belum tertutup, dan runbook menyebutnya terang-terangan:** Log Explorer alat *tarik* — tidak mengirim apa pun. Tanpa penanggung jawab bernama atau kueri terjadwal, runbook ini mewarisi cacat asli BL-18. Tiga bentuk penutup + ongkosnya ditulis; yang termurah (tinjauan manual mingguan) sah **asal ada namanya**, karena "tim akan meninjau" bukan penanggung jawab.
- Sentry ditegaskan **bukan** kandidat: transportnya hanya menerima `error`/`warn` dari klien, dan menaikkan level FR-34 agar lolos ke sana = opsi 1 yang sudah ditolak. Catatan anti-regresi itu ditaruh di komentar `search.ts` supaya orang berikutnya tidak "memperbaikinya" ke arah situ.
- Verifikasi: `npm test` 137 suite / 1718 test hijau, `tsc --noEmit` bersih, lint 0 error, rename-guard clean.

## [2026-07-23] update | BL-19a selesai — form Posisi/Tim tidak lagi membuang field tautan

- Files created: `mobile/src/components/option-picker.tsx`, `mobile/src/app/(app)/__tests__/settings-org-structure-linkage.test.tsx`
- Files updated: `mobile/src/app/(app)/settings-org-structure.tsx`, `DESIGN.md` (§7 token `OptionPicker`), [[feature-gap-backlog]] (BL-19a..d baru)
- **Kelas kegagalan: diam, bukan error.** RPC `create_position` (0022:96) dan `create_team` (0014:491) sudah menerima `p_department_id`/`p_lead_id`, dan `lib`+`hook` sudah mem-forward-nya — hanya layar yang memanggil `createPosition({ name })` dan `createTeam({ name, departmentId: null, leadId: null })` hardcoded. User mengisi struktur org, simpan berhasil, relasinya tidak pernah ada. Tidak ada pesan error yang bisa dilihat QA visual.
- **Diverifikasi lebih dulu, bukan diasumsikan**: ACL kelima RPC dicek di DB staging (`has_function_privilege`) — `authenticated` true, `anon` false untuk semuanya. 0014 hanya menulis `revoke ... from public, anon` tanpa `grant` eksplisit untuk `create_team`, jadi pengecekan ini bukan formalitas. Guard lintas-org untuk `p_lead_id` sudah ditambahkan 0068 — mengaktifkan picker Lead tidak membuka permukaan baru.
- Hanya **departemen aktif** yang bisa dipilih. RPC menerima departemen nonaktif, tapi menautkan struktur baru ke unit yang sudah dipensiunkan hanya memindahkan masalah ke laporan.
- Baris daftar menampilkan tautannya (`Departemen: X`, `Departemen: X · Lead: Y`) — tanpa ini, perbaikan tetap tak terlihat dari UI dan kelas bug yang sama bisa kambuh tanpa ketahuan.
- **Refinement gotcha harness** (memperluas catatan `rn-css-pressable-test-blank-render`): akar gejala "render tes berikutnya kosong" ternyata query **sinkron** tepat setelah `fireEvent.press`. Dengan setiap press diikuti query ber-`await` (`findBy*`), 7 tes press-berat lolos berurutan dalam satu file — tak perlu menyusun ulang urutan tes maupun memecah file.
- Verifikasi: `[A-01..A-07]` 7/7, suite tetangga `fase8-settings-screens` 16/16, `tsc` bersih, lint 0 error.

## [2026-07-23] update | BL-19b selesai — Departemen bisa dinonaktifkan, anggota Tim bisa dikelola (migrasi 0092)

- Files created: `supabase/migrations/0092_org_structure_write_paths.sql`, `supabase/tests/0092_org_structure_write_paths_contract.sql`, `mobile/src/app/(app)/settings-team/[id].tsx`, `mobile/src/app/(app)/__tests__/settings-team-members.test.tsx`
- Files updated: `lib/org-structure.ts`, `hooks/use-org-structure.ts`, `settings-org-structure.tsx`, `components/user-picker.tsx` (`excludeIds`), `lib/database.types.ts` (regen), test linkage (+4 kasus), [[feature-gap-backlog]]
- **Nonaktifkan Departemen TIDAK meng-cascade** ke Posisi/Tim yang menautnya. Ini keputusan, bukan kelalaian: tautan lama justru ALASAN nonaktif dipilih ketimbang delete, jadi "merapikan" dengan `set null` menghancurkan riwayat yang mau diselamatkan. Dikunci `0092-DB-3` dan **diverifikasi merah** dengan varian yang men-`set null` — `position_unlinked` menyala tepat.
- **Gate memakai `create_department`, bukan permission key baru.** Namanya tidak enak untuk aksi nonaktifkan, tapi katalog permission dirujuk literal di CHECK constraint (0017:64/111/116) — menambah key berarti migrasi lintas-tabel demi satu tombol. Utang nama **dicatat, bukan dibayar**.
- **`remove_team_member` memvalidasi keberadaan Tim terpisah dari hasil delete.** Tanpa itu, tim milik org lain dan "orang ini memang bukan anggota" sama-sama menghasilkan 0 baris, dan pesan "bukan anggota" akan mengonfirmasi keberadaan tim lintas-org. Dikunci `0092-DB-6` sebagai uji **anti-oracle** — diverifikasi merah dengan varian tanpa guard: pesannya bocor jadi "Orang ini bukan anggota Tim tersebut".
- **`assign_team_member` ditulis ulang HANYA untuk menambah activity log.** Jejak audit timpang (pelepasan tercatat, penambahan tidak) lebih menyesatkan daripada tidak ada jejak sama sekali. `CREATE OR REPLACE` mempertahankan ACL — bukan DROP+CREATE, jadi grant tidak ter-reset ([[anon-public-rpc-grant-gotcha]]).
- Anggota yang sudah ada **disembunyikan dari picker**: `unique(team_id, profile_id)` pasti menolaknya, dan menawarkan pilihan yang pasti gagal itu jebakan. `UserPicker` dapat prop `excludeIds`.
- **Regen `database.types.ts` dari DB LOKAL, bukan staging** — staging belum punya 0092. Diff terverifikasi tepat 2 fungsi baru, nol perubahan lain, dan CRLF dipertahankan.
- Satu tes dibuang, bukan dibiarkan hijau: `[B-10]` mengklaim membuktikan tombol "Lepas" bukan target a11y bersarang, padahal kartu anggota tidak pressable sehingga aturan itu tidak berlaku di sana — hijau tanpa isi.
- **Suite bertarget hijau ≠ aman.** `DepartmentTab` mulai membaca Posisi/Tim (jumlah tautan di teks konfirmasi), dan mock parsial `@/hooks/use-org-structure` di `fase8-settings-screens.test.tsx` meledak dengan `usePositions is not a function`. Suite tetangga itu **lolos saat dijalankan sebelum perubahan** dan hanya menyala di run penuh — mock parsial pecah ketika komponen menambah pemakaian hook, bukan ketika perilakunya berubah. Jalankan suite penuh sebelum menyatakan selesai; hasil bertarget saja menyembunyikan kelas ini.
- Verifikasi: DB contract **39 lolos, 0 gagal** (naik dari 38), jest penuh **1729/1729** (139 suite), `tsc` bersih, lint 0 error, rename-guard clean.
- **Belum di-apply ke staging** — CI tidak `db push` ([[staging-db-migrasi-tertinggal]]).

## [2026-07-23] update | Merge staging ke PR #169 — dua tabrakan ID diresolusi

- **Tabrakan 1 — nomor migrasi.** `staging` sudah memakai `0089` (PR #165 `search_global_cursor_guard`), `0090`/`0091` (push grants). Migrasi branch ini **dinomori ulang `0089` → `0092`**: `supabase/migrations/0092_org_structure_write_paths.sql` + `supabase/tests/0092_org_structure_write_paths_contract.sql`, label kontrak `0089-DB-1..7` → `0092-DB-1..7`. Konflik ini **tidak muncul sebagai konflik git** (nama file berbeda) — dua migrasi bernomor sama akan berurut lexical (`0089_org…` sebelum `0089_search…`) dan lolos diam-diam.
- **Tabrakan 2 — ID backlog.** `staging` sudah memakai `BL-16`/`BL-17`/`BL-18` (Search). Item branch ini **diganti `BL-16a..d` → `BL-19a..d`**; sufiks huruf tetap dipertahankan karena a/b/c/d memang satu keluarga temuan (idiom yang sama dengan `BL-10a..d`), tapi induknya tidak boleh menabrak ID yang sudah dipublikasikan.
- Yang **tidak** diubah: isi temuan, keputusan produk, maupun assertion mana pun — murni penomoran + berkas terkait.
- §2 triage dimutakhirkan: sisa pekerjaan jadi **tiga item** (BL-18, BL-19c, BL-19d), dan asal-usul BL-19 dibedakan dari trio sisa-utang BL-10.
- **Tabrakan 3 — ketahuan oleh gate, bukan oleh git.** Gate BL-13/BL-17 (mendarat di staging lewat #166) mem-parse `supabase/migrations/*.sql` lalu menuntut tiap `activity_logs.action` ter-emit punya entri di `ACTIVITY_LOG_ACTION_LABEL`. Migrasi 0092 memperkenalkan 4 action baru (`department_activated`, `department_deactivated`, `team_member_assigned`, `team_member_removed`) — nol konflik teks, tapi `[BL17-5]` merah begitu kedua sisi bertemu. Label ditambahkan; ini persis kelas regresi yang gate itu dibangun untuk menangkap.
- Verifikasi pasca-merge: jest **140 suite / 1795 test hijau**, `tsc` bersih, lint 0 error. DB contract **belum** dijalankan ulang di sini (butuh DB lokal); perubahan pada 0092 murni penamaan berkas + label assertion.

## [2026-07-23] update | BL-19c selesai — jalur tulis Profil/Organisasi/Goal (migrasi 0093)

- Files created: `supabase/migrations/0093_self_service_write_paths.sql`, `supabase/tests/0093_self_service_write_paths_contract.sql`, `mobile/src/lib/profile-self.ts`, `mobile/src/app/(app)/settings-profile.tsx`, `mobile/src/app/(app)/settings-organization.tsx`, `mobile/src/app/(app)/goal/edit/[id].tsx`, dua file tes (`settings-self-service`, `goal/__tests__/edit`)
- Files updated: `lib/goals.ts` (`updateGoal` + `GoalPatch`), `lib/org-structure.ts` (`updateOrganization`), `lib/org-timezone.ts` (`orgTimezoneOptions`), `hooks/use-profile.ts` (`useUpdateOwnProfile`), `hooks/use-org-structure.ts` (`useOrganizationActions`), `hooks/use-workspace.ts` (`update` di `useGoalActions`), `goal/[id].tsx` (tombol Ubah), `(tabs)/menu.tsx` (dua entry), `lib/database.types.ts`, [[feature-gap-backlog]]
- **Temuan awal BL-19c terkoreksi.** Catatan aslinya berbunyi "RLS `profiles_update_self` siap sejak 0001 — user tak bisa mengubah namanya sendiri", seolah yang kurang hanya UI. Yang sebenarnya ada di sana adalah **eskalasi hak akses yang bisa dieksploitasi**: policy itu hanya memeriksa `id = auth.uid()`, dan RLS **tidak punya granularitas kolom** — jadi pemilik baris bisa `update profiles set role_template_id = '<template CEO>'` dan langsung memegang seluruh permukaan admin. Varian `organization_id` menembus isolasi multi-tenant yang jadi seluruh alasan 0083 (BL-14) ada; varian `is_active` menghidupkan akun yang sengaja dimatikan admin. Ketiganya **diverifikasi berjalan** di DB dev sebelum perbaikan.
- **Policy DIHAPUS, bukan dipersempit.** Tidak ada bentuk policy yang bisa menutup ini: RLS selalu seluas seluruh baris, jadi "self update" apa pun menanam ulang jebakan yang sama untuk pembaca berikutnya. Penutupnya = ketiadaan GRANT; batas kolom pindah ke badan RPC. Dicabut dari `authenticated, anon` — **bukan** dari `public`, karena itu ikut mematikan `service_role` yang dipakai Edge Function `create-user` ([[service-role-revoke-gotcha]]).
- **Bug ditemukan test, bukan review**: `text[] || 'name'` di `update_goal` membuat parser memilih operator array||array dan mem-parse literalnya sebagai array — `malformed array literal: "name"` **saat runtime, hanya pada cabang yang dijalankan**. `create function` tetap sukses. Semua literal di-cast `::text`.
- **Payload saat Goal terkunci dikirim apa adanya, termasuk `null`.** Mengirim `''` untuk target yang memang null terbaca server sebagai perubahan, sehingga panggilan yang cuma mengganti nama ikut tertolak "Target Goal terkunci". Dikunci `[D-02]`, diverifikasi merah dengan varian yang mengirim string mentah.
- **Periode & target tetap DITAMPILKAN read-only saat Goal aktif**, bukan disembunyikan: field yang lenyap begitu Card aktif terbaca sebagai bug, dan user kehilangan cara melihat nilai yang sedang mengikat perhitungan skornya. Idiom sama dengan BL-02 di `strategy/new.tsx`.
- **Picker zona waktu hanya menawarkan 3 zona Indonesia**, tapi zona tersimpan di luar daftar disisipkan sebagai opsi. Picker yang diam-diam membuang nilai tersimpan akan menimpanya saat disimpan — kehilangan data senyap. Dikunci `[C-07]`, diverifikasi merah.
- **Invalidasi cache menyeberangi lima query key** (`current-profile`, `profile`, `org-profiles`, `profile-detail`, `ranking`): nama user muncul di kelimanya, dan menyegarkan satu saja memberi gejala "tersimpan tapi tidak berubah" yang tampak seperti kegagalan tulis padahal tulisannya sukses.
- Verifikasi: DB contract 0093 **11/11** (DB-3 diverifikasi merah dengan mengembalikan grant+policy pra-0093 — staff berhasil jadi CEO), jest penuh **1811/1811** (142 suite), `tsc` bersih, lint 0 error.
- **Sudah di-apply ke staging 2026-07-23** dan diverifikasi lewat EFEK, bukan `schema_migrations`: grant tulis `authenticated`/`anon` di `profiles` **nihil**, policy tersisa hanya `profiles_select_same_org`, `SELECT` untuk `authenticated` utuh, `service_role` UPDATE **utuh** (revoke menyasar `authenticated, anon` — bukan `public`, lihat [[service-role-revoke-gotcha]]), ketiga RPC `authenticated`-only.
- **Probe eksploitasi dijalankan ulang terhadap staging-schema yang sudah dipatch**, bukan hanya mengandalkan assertion kontrak: `role_template_id` dan `organization_id` sama-sama `permission denied for table profiles`, level aktor tetap `staff`, sementara `update_own_profile` tetap berfungsi termasuk trim-nya. Lubang ini **live di staging selama beberapa jam** antara migrasi ditulis dan di-apply.
- Verifikasi ulang lintas-sesi: DB contract **44/44**, jest penuh **1811/1811** (142 suite), `tsc` bersih, lint 0 error, rename-guard clean.

## [2026-07-23] update | BL-19d selesai — Role Template assignable + Reporting line (migrasi 0094). PRD §34.3 LENGKAP 6/6

- Files created: `supabase/migrations/0094_reporting_line.sql`, `supabase/tests/0094_reporting_line_contract.sql`, `mobile/src/lib/reporting-line.ts`, test `bl19d-reporting-role-template.test.tsx`
- Files updated: `supabase/functions/create-user/index.ts`, `lib/users-admin.ts`, `hooks/use-org-structure.ts`, `settings-org-structure.tsx` (tab "Atasan"), `settings-user-new.tsx`, `lib/database.types.ts` (regen), [[feature-gap-backlog]]
- **Invarian keamanan bagian 1: level efektif diturunkan dari BARIS TEMPLATE DI DB, tidak pernah dari `role_level` kiriman klien.** Kalau level ikut dipercaya dari body, penyerang cukup mengirim template C-Level bersama `role_level: 'staff'` untuk melewati guard eskalasi — template kustom berubah jadi vektor eskalasi, kebalikan dari tujuan fiturnya. Guard eskalasi dipindah agar berjalan SETELAH template diresolusi. Template diambil dengan filter `organization_id`; tanpa itu id template milik org lain diterima dan user baru mendarat memakai role tenant lain.
- **Perilaku lama dipertahankan persis** saat `role_template_id` tidak dikirim: baris seeded tertua per level (`order created_at asc`). Fitur ini menambah pilihan, bukan mengubah default.
- **Cakupan reporting line V1 DESKRIPTIF, bukan otorisasi — dan itu keputusan berdasar temuan, bukan kehati-hatian abstrak.** Kolom `user_permissions.scope` ('own'/'team'/'dept'/'org', 0022) memang ada dan bisa disetel, tapi **tidak ada satu pun policy atau RPC yang membacanya**; klien hanya menampilkannya. Jadi tidak ada mesin scope yang menunggu disambung ke reporting line — menyambungkannya sekarang berarti membangun mesin otorisasi baru sekaligus. Konsekuensi jujur dinyatakan **di UI**, bukan hanya di komentar: menyetel atasan TIDAK memberi akses ke data bawahan (dikunci `[D-05]`).
- **Penjaga siklus ditulis sebagai loop berbatas kedalaman, bukan CTE rekursif.** CTE rekursif atas data yang SUDAH bersiklus akan menggantung lebih dulu sebelum sempat melaporkannya; loop dengan batas 64 gagal keras dan bisa dibaca. Diverifikasi merah dengan penjaga naif satu-tingkat: siklus langsung a↔b tertangkap, tapi **a→b→c→a lolos** — persis kasus yang `0094-DB-3` jaga.
- **FK `ON DELETE SET NULL`, bukan CASCADE** (dikunci `0094-DB-1` + `0094-DB-8`): menghapus atasan tidak boleh ikut menghapus profil bawahannya. Dengan CASCADE, bawahan lenyap diam-diam dan baru ketahuan jauh belakangan.
- **`0094-DB-2` menjaga agar `manager_id` tidak membatalkan 0093**: kalau kelak ada yang "mempermudah" dengan mengembalikan grant/policy UPDATE `profiles`, eskalasi role yang ditutup 0093 ikut hidup lagi. Assertion itu yang menahannya.
- Tab "Atasan" digerbangi `manage_users_permissions`, bukan `manage_settings` — garis pelaporan menyentuh semua orang, bukan satu unit.
...
## [2026-07-24] update | Label migrasi staging diluruskan + indeks trigram terbukti TIDAK drift

### Label `schema_migrations` staging

- Satu baris melenceng: `version 20260723130235` tercatat `0089_org_structure_write_paths` padahal file reponya `0092_org_structure_write_paths.sql` — sisa dari renomor 0089→0092 saat merge staging. Efek schema selalu benar; hanya labelnya salah, dan ia **bertabrakan** dengan `0089_search_global_cursor_guard` yang asli.
- Diperbaiki dengan `update ... where version = ... and name = '<nama lama>'` — dikunci pada KEDUA kolom, jadi mustahil mengenai baris lain; `returning` mengonfirmasi tepat satu baris. Sesudahnya: sisa label lama 0, total baris tetap 92, dan efek schema 0092 diverifikasi utuh (kedua RPC tetap `authenticated`-executable). Riwayat 23–24 Juli kini berurutan `0090 → 0095`.

### `search_global_trigram_indexes_0086_0087` — BUKAN drift, dan dugaan awal SALAH

- Dugaan awal: baris ini "perubahan yang hidup hanya di staging" karena tidak ada file migrasi bernomor itu. **Keliru** — namanya sendiri menjelaskan: `0086_0087` menunjuk migrasi repo yang sudah ada, bukan nomor baru.
- Sebenarnya: **pemutaran ulang bagian indeks dari 0086 dan 0087** ke staging, kemungkinan karena kedua migrasi sempat mendarat tanpa bagian indeksnya — pola yang sudah dikenal di proyek ini ([[staging-db-migrasi-tertinggal]]).
...
## [2026-07-28] update | Sprint 7 (Form & navigasi) 6/6 tiket SHIPPED via PR #221

Sprint 7 pasca-rilis dari [audit produksi 2026-07-26](https://claude.ai/code/artifact/47382ecd-b775-439c-88bc-95cd211d3978) di-merge ke staging (fast-forward, commit `15c0ffb`). 47 file (1438+ / 195-), 2 hook baru; nol tiket memblokir rilis.

- **S7-1 KeyboardAvoidingView** — dibungkus di 13 layar form (`(auth)/login`, `(auth)/reset-password`, `evaluation`, `deadline-change-request`, `manual-score-override`, `settings-team/[id]`, `settings-org-structure`, `settings-notifications-rule`, `settings-card-guidance`, `settings-score-formula`, `settings-strategy-templates`, `settings-permission-users`, `settings-governance-violation`). Pola: `behavior={Platform.OS==='ios'?'padding':undefined}` + ScrollView `keyboardShouldPersistTaps='handled'`. Sebelum Sprint 7 hanya `inbox/[roomId]` yang pakai KAV.
- **S7-2 useDirtyGuard** (hook baru `mobile/src/hooks/use-dirty-guard.ts`) — bungkus `usePreventRemove` dari `expo-router/react-navigation` + `showAlert` cross-platform. Dipasang di 12 form modal (goal/strategy/initiative/action-plan/task new + submit + goal-wizard + evaluation + deadline-change-request + manual-score-override + development-area/problem-statement new). Guard menutup swipe-down iOS (via `preventNativeDismiss`), back Android, dan navigasi lain. Bug pattern awal `submittedRef.current` di render — React Compiler menolak (77 lint error) → diganti `useState<boolean>` yang memicu re-render supaya guard dilepas sebelum navigate sukses. Mock global `expo-router/react-navigation` ditambah di `jest.setup.js` supaya layar tak butuh Stack Navigator di render test.
- **S7-3 LabeledInput.error** — prop `error?: string | null` menampilkan pesan validasi inline per-field (border merah + `<Text role='alert' liveRegion='polite'>` + `aria-invalid` + a11yLabel merangkum pesan; border merah BUKAN satu-satunya sinyal §4 rule 2). Foundation dipropagasikan ke 14 layar: 39 Alert validasi migrasi ke `fieldErrors` state (per-field) atau banner `formError` (untuk validasi non-field). Submit sekarang mengumpulkan SEMUA error field sekaligus, bukan exit di error pertama.
- **S7-4 secureTextEntry + reveal** — prop tambahan di `LabeledInput` dengan tombol reveal built-in (ikon `eye/eye-off-outline`, min 44×44 §4.1, a11yLabel state-eksplisit "Sembunyikan/Tampilkan kata sandi"). Dipakai di `settings-user-new` "Password sementara" (sebelumnya teks biasa terbaca siapa pun di dekat layar admin).
- **S7-5 useSafeBack** (hook baru `mobile/src/hooks/use-safe-back.ts`) — `canGoBack() ? back() : replace('/')` supaya jalur sukses mutasi tidak jadi no-op saat masuk lewat deep-link. Diterapkan di 13 file (14 call site). `(app)/_layout.tsx` menambah 8 registrasi Stack.Screen yatim: `settings-team/[id]` + 6 rute edit Sprint 4 (`{action-plan,development-area,goal,initiative,problem-statement,task}/edit/[id]`) + `task/repeat-edit/[id]`. Hook defensive terhadap mock test lama (`typeof router.canGoBack === 'function'`).
- **S7-6 disabled + konfirmasi ireversibel** — `AckCheckbox` + `WarningCallout` diekstrak dari `finalize-period-modal.tsx` ke `ui.tsx`. `review-submission-panel.tsx`: tombol "Setujui (Selesai)" sekarang membuka mode konfirmasi (WarningCallout + AckCheckbox); `onDecide` hanya dipanggil setelah Ack + tap konfirmasi kedua. `settings-score-formula.tsx`: `handleActivate` → `requestActivate` yang membuka modal konfirmasi; tombol Aktifkan di `DraftEditor` disable saat `isActivating` (aliran `useFormulaActions.isPending`) mencegah dua transisi berturut-turut. `finalize-period-modal.tsx` sengaja tidak di-port ulang — test coverage stabil, hindari kerja tambahan.

**Verifikasi**: 147 test suites / 1929 tests hijau (setelah 8 test kolokasi diupdate untuk assertion inline: `settings-self-service` C-02/C-03/C-08, `strategy/new-period-inherit` BL02-3..5, `task/submit` U10, `goal/edit` D-04); `tsc --noEmit` exit 0; `npm run lint` 0 errors. Semua status check GitHub Actions hijau (Lint/types/tests, Rename guard); DB contract SKIPPED (tak ada migrasi baru).

**Pages updated**: `wiki/log.md` (entry ini). Wiki entities/concepts TIDAK ditambah untuk sprint ini — perubahannya di scope UI/hooks yang isi wiki (permission-model, execution-loop, dsb.) tidak berubah maknanya. DESIGN.md diperbarui: prop `LabeledInput.error/secureTextEntry` + relokasi `AckCheckbox`/`WarningCallout` ke `ui.tsx`.

**Pelajaran durable**: (1) React Compiler menolak baca `ref.current` di scope render, bahkan untuk pola `!ref.current && (...)` di deklarasi state derived — pakai `useState` yang memicu re-render supaya guard hook re-evaluate. (2) `usePreventRemove` butuh NavigationContainer di test env; fix jangka panjang = mock global `expo-router/react-navigation` di `jest.setup.js` (kompromi antara defensive-in-hook dan per-file-mock-update). (3) Ekstraksi komponen konfirmasi ireversibel ke `ui.tsx` menjaga DRY tanpa memaksa refactor situs preseden yang stabil.

## [2026-07-28] update | Sprint 8 (Lokalisasi & performa klien) 6/6 tiket SHIPPED

Sprint 8 pasca-rilis dari [audit produksi 2026-07-26](https://claude.ai/code/artifact/47382ecd-b775-439c-88bc-95cd211d3978) — 6 tiket medium (~32j), fokus rapikan biaya jaringan, pola rapuh di klien, dan tuntaskan lokalisasi Bahasa Indonesia yang tertinggal separuh dari PR #204. Nol tiket memblokir rilis.

- **S8-1 Lokalisasi tuntas** — peta terpusat `METRIC_LABEL` (task_completion→"Penyelesaian Tugas", repeat_compliance→"Kepatuhan Repeat", on_time_rate→"Ketepatan Waktu", review_pass_rate→"Lolos Review", development_contribution→"Kontribusi Pengembangan", governance_discipline→"Disiplin Tata Kelola") + `PEOPLE_TAB_COPY` (Quarter→Kuartal). "Score Formula" → "Rumus Skor" di 8 permukaan (menu, notif deep-link, header layar, layar override, tabs, `_layout.tsx` title). "Governance Violation" → "Pelanggaran Tata Kelola" (tabs, search scope label, access denied). Home headings: "Task Hari Ini" → "Tugas Hari Ini", "Update Terbaru" → "Pembaruan Terbaru". People profile: Achievement Score → "Skor Pencapaian", Breakdown Metrik → "Rincian Metrik", Detail People → "Detail Anggota". `task/[id].tsx`: "Repeat Compliance" → "Kepatuhan Repeat", "On-time" → "Tepat waktu". Strategy breakdown: Quarter/Bulan per Quarter/Total Quarter → Kuartal/Bulan per Kuartal/Total Kuartal. **Tes penjaga baru** `no-english-strings.guard.test.ts` scan JSX literal di `app/`, `components/`, `screens/` untuk 13 frasa Inggris terlarang.
- **S8-2 Signed URL + thumbnail** — `getChatAttachmentSignedUrl(path, opts?)`: default TTL 60s → 600s (sinkron dgn siklus hidup layar chat), opsi `transform` untuk varian thumbnail 440×330 (DPR 2× dari 220 CSS px), opsi `ttlSec` override. `ChatAttachmentThumbnail` di `inbox/[roomId].tsx` sekarang: (1) query thumbnail terpisah dari query full-size, (2) full-size hanya di-fetch saat modal preview dibuka (`enabled: previewOpen`), (3) `<Image onError>` invalidate signed URL saat 403 → refetch tunggal. `staleTime`/`gcTime` diselaraskan (TTL-30s / TTL). Konstanta `CHAT_SIGNED_URL_TTL_SEC` di-export utk konsistensi.
- **S8-3 getUser() cleanup + staleTime default** — QueryClient `defaultOptions.queries.staleTime: 30_000` (sebelumnya jatuh ke 0, memicu refetch tak perlu setiap focus). 10 pemanggilan `supabase.auth.getUser()` di data layer + hook + `menu.tsx` diganti oper `uid: string` dari `useAuth()`: `listMyTasks`, `listPendingReviews` (`cards.ts`); `listPendingInstanceReviews` (`home.ts`); `listNotifications`, `unreadNotificationsCount` (`notifications.ts`); `getMyScore`, `listMyScoreHistory` (`people-score.ts`); `fetchCurrentProfile` (`use-profile.ts`); `fetchProfile` (`menu.tsx`). `home-screen.tsx` + hook wrapper (`useMyScore`, `useMyScoreHistory`, `useNotifications`, `useUnreadCount`) pakai `enabled: !!uid` gate. `org-context.ts` sengaja tak diubah — dipakai di jalur write (mutasi user-triggered), round-trip tunggal per aksi masih akseptabel.
- **S8-4 Chat realtime terfilter + normalisasi rute** — `subscribeChatReads(roomId, myUid, onChange)` sekarang server-side filter `reader_id=neq.<me>`: klien tidak me-refetch pada bacaan sendiri (markRead sudah invalidate lokal), hanya bacaan orang lain. `use-search-messages.ts` menambah filter `member_id=eq.<uid>` di subscription `chat_room_members DELETE`. `markRead()` dua tempat di `inbox/[roomId].tsx` sekarang `.catch(reportError)` (sebelumnya unhandled rejection → Sentry noise setiap kali room dibuka offline). `strategy/new.tsx`: `goalId` route param dinormalisasi (`Array.isArray(raw) ? raw[0] : raw`) mencegah `"a,b"` bocor ke query FK saat deep-link mengirim key ganda.
- **S8-5 Assertion/cast cleanup** — 2 non-null `!` pada `auth.user` (`use-profile.ts:53`, `menu.tsx:155`) hilang otomatis dari S8-3 (getUser + assertion sekaligus). 2 cast `rpc as unknown as` di `card-rules.ts` (upsertCompletionRule/upsertCardGuidance) dihapus — kedua RPC sudah ada di generated `database.types.ts`, cast justru mematikan type-check nama & argumen (`p_reason: null` → `undefined` supaya cocok dgn signature `string | undefined`). `settings-score-formula.tsx:219` `sum` computation pakai `Number.isFinite` (samakan pola dgn baris 84). `people-profile/[id].tsx:117` sparkline pakai helper `effectiveScore` + `.filter(Number.isFinite)` — user tanpa skor sekarang jadi gap di chart, bukan titik 0 palsu.
- **S8-6 Housekeeping** — `storage.ts:43` regex byte kontrol literal (`\x00-\x1F\x7F`) diganti ke escape string; komentar JSDoc sudah menjelaskan maksud tapi source-nya biner di git blame. TZ organisasi diseragamkan lewat `lib/format-datetime.ts` baru (single-source `formatDateTime` + `formatDateTimeIdMedium`); `activity-log-panel.tsx` sekarang pakai `useProfile().profile?.org_timezone` (sebelumnya `toLocaleString('id-ID')` tanpa `timeZone` → jatuh ke tz perangkat, event yang sama tampil beda jam di dua panel). `submission-card.tsx` re-export `formatDateTime` dari lib baru supaya pemanggil eksisting tak perlu diubah. **Modul mati**: scout verifikasi negatif — semua ~65 file `lib/*.ts` punya ≥1 importer non-test.

**Verifikasi**: 147 test suites / 1932 tests hijau setelah 12 file test diupdate (hook wrapping baru butuh mock `@/providers/auth-provider`; test yang mengunci copy lama menjadi copy baru; `storage.test.ts` disesuaikan dgn signature `opts?` + default TTL 600s); `tsc --noEmit` exit 0; `expo lint` 0 error (76 warning baseline pra-Sprint). Guard test `no-english-strings.guard.test.ts` menegakkan 13 frasa terlarang tidak muncul di JSX text di `app/`, `components/`, `screens/`.

**Pages updated**: `wiki/log.md` (entry ini). Wiki concepts/entities tak diubah — perubahan Sprint 8 adalah lokalisasi copy + refactor internal, semantik model tetap.

**Pelajaran durable**: (1) `useAuth()` di hook wrapper daripada `auth.getUser()` di data-fn menghilangkan 1 round-trip per query — untuk beranda dgn 8 useQuery yang refetch on-focus, penghematan nyata. Pola pengoperan `uid` sudah presedennya di `search.ts` (jsdoc menyebut alasan). (2) Realtime filter `neq.<self>` menyingkirkan echo-diri tanpa bergantung pada denormalisasi kolom. (3) Escape byte kontrol (`\x00-\x1F\x7F`) di source lebih portable daripada literal — teks source tetap terbaca di semua editor + diff/blame. (4) Timezone UI wajib punya sumber tunggal (`lib/format-datetime.ts`) — dua jalur berbeda (device tz vs org tz) di dua komponen render event yang sama membuat pengguna melihat jam berbeda di layar yang sama.

## [2026-07-28] update | Sprint 9 (Utang tes & kebersihan basis data) 7/7 tiket via PR #224

Sprint 9 pasca-rilis dari [audit produksi 2026-07-26](https://claude.ai/code/artifact/47382ecd-b775-439c-88bc-95cd211d3978) — 7 tiket (~35j), fokus mengarahkan jaring pengaman ke area yang benar-benar tak terjaga (retensi, drift tipe DB, perilaku vs regex source, ownership contract terkarantina). Nol tiket memblokir rilis. PR #224 ke `staging`, branch `sprint9/test-debt-cleanup`.

- **S9-1 Ownership contract terkarantina** — `WIP_REPAIR_BACKLOG.md` kini punya pemilik + target sprint eksplisit untuk 13 file `.wip.sql`. Tiga cluster shared-root diidentifikasi: **A** (`0018/0040/fase3/ws3b` — `initiatives.strategy_id` NOT NULL post rename 0045), **B** (`0019/fase4` — `kpi_areas` dropped → `strategies`), **C** (`0038/fase8` — `raise 'ROLLBACK_OK'` sentinel runner-incompatible). Sisa 5 file individual. Rencana batch: Cluster A dulu di Sprint 10 (satu template repair diterapkan ke 4 file), lalu B & C. AC "yang tersisa punya pemilik dan tanggal eksplisit" terpenuhi tanpa memperbaiki isi test — file `.wip.sql` sendiri sengaja tidak disentuh di sprint ini.
- **S9-2 Fixture mesin skor 6-metrik** — `supabase/tests/0116_sprint9_score_six_metric_breakdown_contract.sql`. Sebelumnya fixture `0079` + `close_period_snapshot_contract` HANYA memakai satu metrik (`action_plan_completion`); bug 0099 (`compute_development_contribution` merujuk `initiatives.problem_statement_id` yang hilang post rename 0045) hanya muncul bila formula MEMUAT metrik rusak — dan tak ada tes yang melakukannya. Nilai `metric_breakdown` juga tak pernah di-assert (fixture eksisting seed `'{}'::jsonb`). Dua blok baru: **T9-2-1** seed formula 6-metrik zero-data → assert 6 kunci hadir (`apc/rc/otr/rpr/dev/gov`) dengan nilai default per metrik (ratio → 0, gov → 100), `auto_calculated_score = 10.00`. **T9-2-2** seed satu `governance_violation` severity `critical` → gov=60, score=6.00 (membuktikan `compute_governance_discipline` benar-benar terpanggil bukan sekadar default no-op).
- **S9-3 Drift check `database.types.ts` + titik-buta test** — 3 perubahan: (a) `mobile/package.json` menambah script `gen:types` = `supabase gen types typescript --local > src/lib/database.types.ts` (sebelumnya command manual di README, tak pernah otomatis); (b) job `db-contract` di `ci.yml` menambah step yang regen types dari live schema (post `supabase start`) lalu `diff -u` terhadap file committed — beda apa pun = build merah dengan error pointer ke `npm run gen:types`. Filter `changes.db` juga menambah `mobile/src/lib/database.types.ts` sebagai trigger supaya PR yang edit types-file tapi tak menyentuh migrasi tetap ter-cover; (c) test langsung baru untuk **`getOrgContext`** (5 tes: happy path, unauth, data auth kosong, profil null, organization_id null) dan **root `_layout.tsx`** (3 tes smoke: spinner saat `initializing=true`, Stack render saat `initializing=false`, tema dark tanpa crash). Root layout sebelumnya tidak punya test sama sekali — regresi hanya terdeteksi saat device build. RTL v14 `render()` mengembalikan Promise → semua render harus `await` (bug awal: `screen.getByTestId` throw "render function has not been called" saat render sync).
- **S9-4 Perilaku vs regex source** — `supabase/tests/0117_sprint9_confidential_access_behavior_contract.sql`. Menggantikan pola brittle `pg_get_functiondef(...) like '%confidential_access_rules%'` (0077 T4-T6) yang bisa lolos meski implementasinya salah (menyebut tabel di komentar, di cabang mati, atau membalik gerbangnya). Tiga blok baru memanggil `can_access_goal/strategy/initiative` dari 3 aktor (staff tanpa rule → assert `false`, staff dgn rule di allowlist → assert `true`, CEO → assert `true` bypass) dan memverifikasi boolean hasilnya. 0077 T4-T6 dipertahankan sebagai jaring pengaman kedua dengan catatan penunjuk ke 0117. Sisa contract regex-source (0067/0068 cross-org, 0079 T-DB-4/5 advisory-lock, 0084 notification-type, 0085/0087 search-global) belum dikonversi — didokumentasikan sebagai backlog konversi bertahap.
- **S9-5 Retensi mengejar volume** — `supabase/migrations/0116_sprint9_retention_batch_loop.sql`. `purge_old_activity_logs` di-rewrite: alih-alih single batch 10k, sekarang loop internal `until v_batch < p_batch_size or v_iter >= p_max_batches` (default 500 iter × 10k = 5M baris/run). Sebelumnya arrival rate 94 write-site melampaui 10k/hari begitu tenant tumbuh — window 12 bulan tak akan pernah tercapai. Tabel `notifications` (0008) yang sebelumnya belum punya kebijakan retensi apa pun sekarang punya `purge_old_notifications` (default 6 bulan, lebih pendek dari activity_logs karena notifikasi lebih ephemeral) + `pg_cron` schedule nightly `30 20 * * *` (30 menit setelah purge activity_logs, menghindari kontensi write pool). Signature 4-arg (`p_retention_months, p_batch_size, p_activate_after, p_max_batches`) — activate_after default `2026-08-27` (30 hari observasi baseline sebelum delete pertama).
- **S9-6 search_cards trigram + LIMIT** — `supabase/migrations/0117_sprint9_search_cards_ilike_limit.sql`. `search_cards` (versi live di 0046) memakai `lower(name) LIKE lower(v_q)` di 7 cabang UNION ALL — planner TIDAK memilih indeks trigram GIN yang dibangun di 0096 pada kolom `name` raw (`name extensions.gin_trgm_ops`) karena predikat `lower(name)` tak match opclass. Semua cabang seq-scan. Fungsi juga tanpa `LIMIT` — query "ana" 3 huruf umum bisa mengembalikan ribuan baris. Fix: (a) ganti `lower(name) LIKE` → `name ILIKE v_q_ci` (opclass `gin_trgm_ops` mendukung ILIKE case-insensitively secara internal), (b) tambah param `p_limit int default 50` cap keras 200 (mirror pola `search_global` 0089), (c) signature 3-arg klien tetap kompatibel via default arg — `database.types.ts` diupdate menambah `p_limit?: number`. `drop function ... cascade` diperlukan karena tambah param → REVOKE public/anon + GRANT authenticated eksplisit di akhir.
- **S9-7 Dokumen + npm ci + eas-cli pin** — 3 kelompok fix: (a) **Dokumen** — `README.md:422` "deploy staging di-gate oleh job quality yang sama" ditulis ulang (deploy staging tak lagi rerun quality — comment `deploy-staging.yml` L25-33 sudah bilang begitu; gate CI = required PR check + branch protection). `README.md:434` `.env.production` diganti "EAS Dashboard sesuai runbook P3-D". `README.md:125` tambah catatan bahwa `.env.*` untuk dev lokal saja. `mobile/AGENTS.md:57` mention "CircleCI post-merge tetap gate" diganti "GitHub Actions `ci.yml` post-merge tetap gate" (`.circleci/config.yml` sudah dihapus 2026-07-25). (b) **npm ci** — 3 workflow (`ci.yml`, `deploy-staging.yml`, `release.yml`) flip `npm install` → `npm ci` dengan **prasyarat operator jalankan `regen-lockfile.yml` (workflow_dispatch) lebih dulu** bila lockfile Windows-generated drift di optional wasm deps (`@emnapi/*`, `lightningcss-*`). Ini prasyarat merge PR #224. (c) **eas-cli pin** — 3 spot `npm install -g eas-cli` diganti `npm install -g eas-cli@^16` (rentang major agar deploy reproducible tapi patch security bisa masuk).

**Verifikasi**: 150 test suites / **1940 tests hijau** (+8 dari baseline Sprint 8 = 1932: 5 org-context + 3 root layout smoke); `tsc --noEmit` exit 0; `expo lint` 0 error (76 warning baseline). Pre-push hook (`--changedSince origin/staging`) lolos.

**Prasyarat merge PR #224**: operator jalankan `Regenerate lockfile (Linux)` workflow_dispatch dulu sebelum merge — jika tidak, CI gagal di step `npm ci`. Setelah PR regen tersebut merged, PR ini bisa direbase & merged.
...
## [2026-07-30] update | Arsipkan dokumen V1.82 superseded, bersihkan noise repo

- File dipindah ke `archive/` (via `git mv`, riwayat terjaga): `prd/01-konsep-dan-fondasi.md`, `prd/02-spesifikasi-card-dan-eksekusi.md`, `prd/03-sistem-permission-data-governance.md` → `archive/prd-v1.82/`; `BUILD-PLAN.md` → `archive/BUILD-PLAN.md`; `MENU_UI_LOCK_SPEC_V1.82.md` → `archive/MENU_UI_LOCK_SPEC_V1.82.md`. Banner "[ARSIP — digantikan]" ditambahkan di tiap file + `archive/README.md` baru menjelaskan pemetaan supersede.
- Verifikasi sebelum arsip: `MENU_UI_LOCK_SPEC_V1.82.md §22.6` (klaim menang atas PRD untuk area Menu) dicek langsung ke kode — `menu.tsx` sudah direstrukturisasi ikut PRD V1.83 §31 sejak commit `a1dfdee` (sprint8, 2026-07-28). Konflik yang dicatat `overview.md` (audit 2026-07-15) sudah selesai secara kode, catatannya yang basi.
- Pages updated: `overview.md` (gap-list item Menu ditutup; catatan "prd/ belum disinkron" diperbarui jadi "sudah diarsipkan"), `index.md` tidak berubah (tidak pernah menaut prd/BUILD-PLAN).
- File lain dibersihkan (bukan wiki, noise repo): 3 folder worktree kosong tanpa `.git` di `.claude/worktrees/` (sisa cleanup sebagian, tak terdaftar di `git worktree list`) dihapus + `git worktree prune`.
- Link diperbaiki mengikuti pemindahan: `README.md` (5 tempat), `mobile/README.md`, `.claude/workflows/sdd-plan.js` (konteks sumber spec untuk `/sdd-plan` diarahkan ke `PRD.md` + `wiki/log.md`, bukan lagi `prd/`+`BUILD-PLAN.md`).
- Tidak diarsipkan meski awalnya dicurigai: `specs/` (arsip beku by-design, dirujuk migration SQL — sudah didokumentasikan sendiri), `design.html` (masih dirujuk aktif oleh `DESIGN.md` + wiki), `ui/` (sudah gitignored, tak pernah masuk repo).
- Key takeaways: (1) klaim "belum selesai" di wiki bisa basi — sebelum mengarsipkan dokumen yang ditandai sebagai konflik aktif, verifikasi ke kode/commit terbaru, jangan percaya catatan lama begitu saja. (2) Trio dokumen bertaut (`prd/01↔02↔03`) diarsipkan sebagai satu set meski hanya sebagian yang self-declare stale, supaya link internal tidak putus.

## [2026-07-30] update | Status card tree Workspace jadi badge eksplisit

- **Bug**: Goal berstatus DB `draft` menampilkan "Aktif · Target …" di baris meta tree Workspace. Kata "Aktif" itu output `WS_TREE_COMPACT_COPY.periodState(past)` (`mobile/src/lib/workspace-copy.ts:30`) — artinya "periode belum lewat", BUKAN status siklus-hidup card. Pola bentrok makna yang sama dengan bug orb hub-card yang ditutup PR #226.
...
## [2026-07-30] update | Rollup progress rekursif Initiative & Action Plan (Opsi B, 0118)

- **Fitur**: level Action Plan & Initiative di `workspace_card_progress` beralih dari `%-anak-done` ke **rata-rata tak berbobot progress anak** (meniru heuristik klien `progress.ts`). Leaf Tugas: one_time = mapping status (draft/archived/cancelled 0, assigned 10, revision 30, in_progress 50, submitted 80, done 100); repeat = Repeat Compliance `round(100*done/total)` atas instance non-archived (all-time, tanpa `submitted_late`, tanpa period-scoping). AP = avg(Tugas); Initiative = avg(AP). Presisi numeric antar level, `round()` hanya di output (OQ-3).
- **Isolasi**: hanya pid Initiative/AP yang beralih. Node type `action_plans` dirollup beda per induk — Initiative→avg rekursif, Problem Statement→count-done. Goal/Strategy attainment + Development Area tak tersentuh; disalin byte-for-byte dari 0102. `is_measured=true` hanya untuk attainment.
- **Keamanan**: tetap `SECURITY INVOKER` + `search_path=''` + return shape sama → `database.types.ts` byte-identik (gate drift CI hijau), ACL `authenticated`-only. RLS `task_instances`/`task_submissions` (gerbang `can_access_task`) menjaga instance rahasia tak bocor ke rata-rata (agregat parsial per pemanggil, diuji level-instance).
- **DB**: migrasi `0118_workspace_progress_recursive_rollup.sql` (CTE `relevant_ap`/`task_progress`/`ap_progress`/`initiative_progress` + coalesce assembly). Kontrak `supabase/tests/0118_recursive_rollup_contract.sql` — 14 blok (governance introspeksi, AC-1/2/3/4/9/11/12/14/16/17/18/19, attainment byte-for-byte, kebocoran confidential level-instance, mixed-batch). RED vs 0102 → GREEN vs 0118.
- **Klien**: header detail Inisiatif & Rencana Aksi (`initiative/[id].tsx`, `action-plan/[id].tsx`) menarik angka orb dari RPC via `useCardProgress` (label "Progress" via `treeOrbLabel`), fallback ke `ratioDoneOfChildren` saat RPC belum termuat — sinkron dengan orb tree.
- **CI**: PR [#228](https://github.com/nawawi489/RencanApp/pull/228) → `staging`. DB contract + lint/types/tests + rename guard hijau.
- **Pages updated**: [[workspace-card-progress]] (bagian baru "Rollup rekursif ... Opsi B"), [[action-plan]] (progress kartu), [[index]], `[[log]]`.
- **Pelajaran durable**: spec/TDD-plan `workspace-progress-recursive-rollup-*` tak ada di repo mana pun; formula tetap terderivasi penuh dari sumber otoritatif live (badan 0102, `progress.ts` sbg SoT mapping OQ-1, `can_access_task`). Worktree awal ter-pin ke lineage `main` (migrasi ≤0071) yang divergen dari `staging` (≤0117) — DB lokal justru di lineage staging (@0103); rekonsiliasi: branch baru dari `origin/staging` + apply migrasi lokal 0104→0117 sebelum authoring 0118.

## [2026-07-30] update | Benchmark 0118 vs 0102: tidak ada regresi (+ gotcha ANALYZE)

- **Konteks**: item Wave 3 yang sebelumnya sengaja dilewatkan (deferred) saat rollup rekursif 0118 di-ship — validasi performa `workspace_card_progress` setelah CTE rekursif Initiative/AP ditambahkan.
- **Metode**: `scripts/ops/benchmark-workspace-card-progress-0118.sql` — seed org disposable (~210 initiative, ~1.6K AP, ~12K task, ~48K instance), timing 0118 vs 0102 pada data identik di 8 skenario batch (digrounding dari pemanggilan nyata `use-workspace.ts`: batch Initiative/AP selalu "anak satu parent expanded", kecil; hanya Goal/Development-Area fetch flat org-wide, level yang tak tersentuh 0118).
- **Temuan pertama (SALAH, dikoreksi)**: tanpa `ANALYZE` eksplisit tepat setelah bulk-seed ~62K baris, 0118 terlihat 7–20x lebih lambat dari 0102 (satu initiative tunggal 5 detik). Setelah `ANALYZE` dijalankan sebelum timing, 0118 dan 0102 **statistik identik** di semua 8 skenario — `EXPLAIN (ANALYZE, BUFFERS)` kasus terberat mengonfirmasi 59.016 buffer hit SAMA PERSIS untuk kedua badan fungsi.
- **Kesimpulan**: rollup rekursif 0118 **tidak** memperkenalkan regresi performa, termasuk pada stress case (semua 210 initiative / 1573 AP org sekaligus — bukan traffic UI nyata, karena batch Initiative/AP klien selalu dibatasi anak satu parent).
- **Script**: dibuat self-contained (embed badan 0102 untuk swap sementara + badan 0118 untuk restore, bukan bergantung pada langkah bash eksternal) dengan guard `-v confirm_local=yes` wajib (mencegah pemakaian tak sengaja di staging/produksi — script ini bulk-seed + swap fungsi LIVE), dan verifikasi integritas (`raise exception` bila hash restorasi tak cocok) sebelum cleanup.
- **Pages updated**: [[workspace-card-progress]] (bagian baru "Benchmark (0118 vs 0102)" + gotcha ANALYZE), `[[index]]`, `[[log]]`.
- **Pelajaran durable**: timing tepat setelah bulk-insert TANPA `ANALYZE` eksplisit dapat melaporkan regresi berat yang sama sekali tidak nyata (planner bekerja dari statistik stale/tak-ada, mungkin juga berlomba dengan autovacuum background) — selalu `ANALYZE` sebelum timing pada data yang baru di-seed, atau risikonya adalah kesimpulan performa yang salah total.

## [2026-07-31] update | RETRACTION: benchmark 0118 vs 0102 — realistic OK, stress-case ~7x slower

- **Konteks**: entry sebelumnya `[2026-07-30] Benchmark 0118 vs 0102: tidak ada regresi` **DITARIK SEBAGIAN**. Reproduksi ulang beberapa kali menunjukkan angka "no regression" di run pertama adalah **anomali single-run yang tak reproduce-able** — bukan kebenaran.
- **Cara ketahuannya**: setelah commit script + wiki claim awal, script committed di-run end-to-end untuk verifikasi guard + integrity check. Run itu MEREPRODUKSI ~100 detik untuk stress case (batch=1573 AP) — sama seperti "run pertama sebelum ANALYZE" yang saya kira sudah dijelaskan sebagai artefak stale-stats. Diagnostic isolasi (3 iterasi `ANALYZE` + `EXPLAIN` back-to-back atas data identik) mengonfirmasi lambatnya reproducible, bukan noise, dan 0102 pada kondisi cold-identik tetap ~15 detik — jadi rasio ~7x nyata, bukan artefak.
- **Root cause struktural** (dari `EXPLAIN` internal via `WITH ... AS MATERIALIZED` probe atas badan CTE 0118): 97% runtime stress case dihabiskan `Index Scan on task_instances` dengan filter `can_access_task(task_id)` — RLS SECURITY DEFINER dipanggil per baris instance (45K+ evaluasi karena `task_progress` LEFT JOIN `task_instances`). 0102 tak pernah menyentuh `task_instances`, jadi tak membayar biaya ini.
- **Percobaan mitigasi (GAGAL)**: `WITH … AS MATERIALIZED` pada 4 CTE rekursif (`relevant_ap`, `task_progress`, `ap_progress`, `initiative_progress`) — **tak menurunkan waktu stress case** (batch=1573: 103s materialized vs 104s baseline; batch=1: 634ms vs 642ms). Implying planner sudah pilih materialization internal atau bottleneck-nya `can_access_task` per-instance yang materialization tak sembunyikan.
- **Impact assessment**: `useCardProgress` klien (`use-workspace.ts`) SELALU batch "children of one expanded parent" (single/low-double-digit). Batch UI nyata (single card ~640ms, satu Strategy expanded ~470ms, satu Initiative expanded ~600ms) semua wajar dan **tidak terpengaruh**. Regresi stress-case adalah **latent risk** kalau UI di masa depan pernah mem-batch flat org-wide Initiative/AP — bukan degradasi UX yang teramati sekarang.
- **Keputusan**: rilis 0118 TIDAK di-rollback (sudah merged, UI tak terpengaruh). Optimasi lebih dalam (guard visibility di level AP dulu, atau ambil `task_id` set ter-authorize dalam satu pass sebelum sentuh instances) di-defer ke investigasi terpisah.
- **Bug script**: hash-based integrity check gagal karena hand-copied restore body menghilangkan komentar SQL yang tersimpan sebagai bagian `prosrc` — restore body sekarang byte-exact dari file migrasi 0118. Committed dengan script; pelajaran ditambahkan sebagai callout di [[workspace-card-progress]].
- **Pages updated**: [[workspace-card-progress]] (bagian benchmark direwrite total: rasio jujur, root cause, mitigasi gagal, callout hand-typed prosrc), `[[log]]` (entry ini).
- **Pelajaran durable jujur**: (1) **satu run hijau tidak cukup untuk klaim performa** — reproduksi 3+ kali sebelum menulis "tidak ada regresi"; (2) `LANGUAGE SQL prosrc` menyimpan komentar verbatim → hand-typed restore body akan hash beda walau logic identik → integrity check via `pg_get_functiondef` md5 wajib, dan restore selalu dari file migrasi otoritatif; (3) SECURITY DEFINER helper di dalam RLS `USING` clause bisa jadi bottleneck O(N) atas rows yang di-scan — beda cabang query yang tak menyentuh tabel yang di-guard punya profil performa fundamental berbeda; (4) skill diagnostic reset setelah menemukan artefak: JANGAN buat kesimpulan definitif dari run pertama yang "kebetulan cocok" dengan hipotesis mitigasi.

## [2026-07-31] update | Rollup rekursif 0118 (Opsi B) — Wave 3–7: invalidasi submit + tes orb detail + supersede doc

- **Konteks**: penutup fitur "Rollup Progress Rekursif Initiative & Action Plan (Opsi B)" pasca Wave 0-2 (migrasi 0118 + kontrak, [PR #228](https://github.com/nawawi489/RencanApp/pull/228)) & benchmark (#229). Wave 3–7: satu gap invalidasi klien + regresi tes + rekonsiliasi dokumentasi. Basis di `origin/staging` (branch worktree awal jauh divergen; di-rebase ke staging dulu).
- **Wave 3 (verifikasi)**: kontrak `supabase/tests/0118_recursive_rollup_contract.sql` re-run **14/14 blok PASS** di DB lokal. Migrasi 0118 sudah rapi (komentar per-level, presisi numeric round-di-output, eksklusi archived konsisten) & sudah merged/applied → immutable, tak diubah. Benchmark sudah tercatat (#229) + `EXPLAIN (ANALYZE)` segar single-initiative = **25.5 ms** (≪ `statement_timeout` 8s `authenticated`). Indeks `idx_task_instances_task` **sudah ada** & bukan bottleneck (RLS `can_access_task` per-baris yang mahal) → **tak ada migrasi 0119** (index baru contraindicated).
- **Wave 4 (submit invalidation, RED→GREEN)**: `use-submission.ts` `onSuccess` tak menyegarkan orb rekursif. Tambah `[H14][H15]` di `use-submission.test.tsx` (RED: 2 gagal) → tambah `qc.invalidateQueries({ queryKey: ['workspace_card_progress'] })` **bare-prefix** (panjang 1, prefix-match semua batch `['workspace_card_progress', ids]`). Submit Tugas menggeser leaf `task_progress` → rata-rata rekursif AP & Initiative; tanpa ini orb tree/detail basi sampai refetch lain.
- **Wave 5 (tes orb detail)**: header detail Initiative & Action Plan sudah di-wire ke RPC via `useCardProgress` (`progressOf(id) ?? ratioDoneOfChildren(...)`, label `treeOrbLabel`) di #228. Tambah tes dedikasi `initiative/__tests__/detail-progress-orb.test.tsx` & `action-plan/__tests__/detail-progress-orb.test.tsx` (AC-20: RPC menang atas count-done; OQ-5: `progressOf=0` menang via `??` bukan `||`; fallback count-done saat RPC null; loading/error tak flash nilai salah). Mock `useCardProgress` sudah ada di kedua suite `mbr-completion`.
- **Wave 6 (audit surface)**: grep `ratioDoneOfChildren`/`ratioActiveOfChildren` — **tak ada surface lain** yang menderivasi orb Initiative/AP independen. Hanya dua detail page itu yang render orb Initiative/AP, keduanya RPC-first. Tree Workspace pakai `useCardProgress` (RPC). `development-area/[id].tsx` derive count-done klien **tapi level DA (bukan Init/AP)** & semantik identik cabang RPC-nya — sesuai guardrail, tak diubah. Nol divergensi tree-vs-surface.
- **Wave 7 (doc)**: [[workspace-card-progress]] + callout **supersede eksplisit keputusan owner 2026-07-03 poin 1** ("rumus induk = %anak langsung done non-rekursif") untuk Initiative/AP. [[workspace-progress-orb-tdd-plan]] ditandai *sebagian superseded* (banner + strike poin 1). [[card-model]] +bagian "Progress card (orb)". `ratioDoneOfChildren` **dipertahankan** di `progress.ts` (fallback Init/AP + rumus induk Goal/Strategy/DevArea).
- **Pages updated**: [[workspace-card-progress]], [[workspace-progress-orb-tdd-plan]], [[card-model]], [[log]].
- **Pelajaran durable**: (1) "Wave 0-2 selesai" bisa berarti lebih jauh dari asumsi handoff — #228 sudah mencakup wiring UI detail + fix mock, jadi Wave 5 "RED" jadi GREEN-konfirmasi, bukan drive; verifikasi state NYATA sebelum menulis tes merah. (2) Invalidasi cache rekursif wajib **bare-prefix**, bukan key ber-id — satu mutation leaf memengaruhi orb di banyak level/batch.

## [2026-08-01] update | WS-05 Period Focus — scope Goal ke TAHUN fokus (Opsi A) + copy empty-state jujur

- **Konteks**: perbaikan celah period-focus di Performance/Development Workspace tree. Arah desain **Opsi A (PRD-patuh, §11.1/§7.6)**: Goal bersifat **TAHUNAN** & selalu tampil sebagai konteks; period switcher (bulan/quarter) men-scope **turunan**, dan dimensi **TAHUN** men-scope Goal. Owner mengonfirmasi tak ada konsep "goal bulanan"; seed lokal sudah dibetulkan jadi goal tahunan. Basis `origin/staging` (branch worktree awal tertinggal — tak dipakai).
- **Celah #1 (BUG, fixed TDD)**: `listGoals()` menarik SEMUA goal tanpa filter → goal tahun lain (mis. 2025) **bocor** ke tree saat fokus 2026. Fix: pure helper `overlapsFocusYear(card, year)` di `lib/period-focus.ts` (scope **hanya dimensi TAHUN**: `period_start <= 31 Des tahun-fokus AND period_end >= 1 Jan tahun-fokus`; kartu **tanpa periode = selalu tampil**, konsisten `cardPeriodStatus`→'current'; batas null = tak-terbatas; date-only leksikografik), difilter **client-side** (`useMemo`, aman React Compiler) di `usePerformanceItems` + `useDevelopmentItems` (`screens/workspace-screen.tsx`) memakai `focus.year`. Development Area **dicerminkan**. **Hub lobby sengaja TIDAK di-scope** → hitungan hub-card tetap total lintas-waktu (tak pecah).
- **Celah #2 (fixed)**: `WS_COPY.emptyGoalTitle` lama `'Belum ada Goal aktif di periode ini.'` menyiratkan scoping periode-level yang (per Opsi A) tak ada → diganti **`'Belum ada Goal di tahun ini.'`** (+ `emptyDevAreaTitle` senada). Lulus guard `no-english-strings`.
- **Celah #3 (INVESTIGASI → escalate, TIDAK diubah)**: verdict **bukan bug**. §11.3 justru **MEWAJIBKAN** turunan periode-lewat tetap **TAMPIL** (badge "Periode lewat" + Detail bisa dibuka + tombol tambah dinonaktifkan) — persis perilaku sekarang. §7.7 + owner decision 2026-07-03: dim dicabut, badge sinyal tunggal, larangan +turunan tetap. §7.6 "tidak menampilkan semua turunan tahunan sekaligus" = **progressive disclosure** (tree ringkas + expand panah, §14) — sudah ada. Menyembunyikan turunan akan **melanggar §11.3** → perubahan perilaku, butuh keputusan produk via `/sdd-plan`.
- **Tes**: RED→GREEN pure helper (`lib/__tests__/period-focus.test.ts`, blok `overlapsFocusYear`) + bukti UI kebocoran lintas-tahun (`workspace.test.tsx` `[S1-2/WS-05]`: goal 2025 hilang, 2026 tampil; `[WS-05·null]`, `[WS-05·empty]`). Tiga tes lama yang mengunci perilaku bocor (`S1-2`, `S3-3`, `W08·1`) diperbarui ke model tahunan — lock "+" & badge "Periode lewat" pada Goal kini via **fokus arsip**, bukan periode 2025 milik card (Goal tahunan tak pernah 'past' via periodenya sendiri di dalam tahun fokus).
- **Gate**: **1963 jest hijau** (152 suites) · type-check bersih · lint 0 error · pre-push lolos. PR [#234](https://github.com/nawawi489/RencanApp/pull/234) → `staging`. Murni data-layer query (client) + copy; tak menyentuh RPC DB/attainment.
- **Pages created**: [[ws-05-period-focus-year-scoping]] (concept page — Opsi A, dua dimensi periode, Celah #1–#3, interaksi engine, implikasi tes).
- **Pages updated**: [[workspace]] (bagian "Period focus"), [[ws-04-governance-debt]] (Related), [[index]], `[[log]]` (entry ini).
- **Pelajaran durable**: (1) tes bisa **mengabadikan perilaku bocor** — `[S1-2]` lama mengunci "goal 2025 tampil dgn badge Periode lewat saat fokus 2026", justru premis bug; audit tes yang mengunci literal saat memperbaiki scoping. (2) "scope periode" punya **dua dimensi** (tahun vs bulan/quarter) — Opsi A men-scope Goal HANYA di dimensi tahun; jangan pukul rata ke sub-periode (goal tahunan wajib tampil sepanjang tahunnya). (3) badge "Periode lewat" pada Goal tahunan mustahil via periodenya sendiri (annual card selalu overlap tahun fokus) — hanya lewat **fokus arsip** (`isAddLocked`→`focusPeriodStatus`); relokasi tes ke mekanisme itu.
- **Pages updated**: `wiki/entities/card-model.md` (sub-bagian baru "Status card di tree Workspace"), `wiki/log.md` (entry ini). `wiki/index.md` tak diubah — halaman entities sudah terdaftar.
- **Pelajaran durable**: parameter yang diterima tapi tak dirender di UI = kontrak diam-diam bocor; grep TOKEN telanjang (`statusLabel:`) menemukan 7 call site dalam sekali sapu. Rename `'Aktif'` → `'Periode berjalan'` di *sumber* label (bukan sekadar tambah badge) memutus akar bentrok makna, meski cost-nya menyentuh test yang mengunci literal — untungnya assertion tes memakai regex `/Periode lewat/` (bukan `/Aktif/`), jadi selamat.

## [2026-07-30] query | Login lokal ceo@rencan.local gagal → kredensial wiki basi

- **Konteks**: user minta coba login `localhost:8091/login`. Wiki [[kredensial-login]] merujuk `ceo@rencan.local` / `rencan123` (verifikasi terakhir 2026-07-06).
- **Temuan**: login `ceo@rencan.local` gagal — "Email atau kata sandi salah." Verifikasi langsung ke DB lokal (`docker exec supabase_db_supabase psql`): `auth.users` hanya berisi **3 baris**, domain `@fixtures.local` (`ceo`, `dcr.ceo`, `dcr.reviewer`) — bukan 6 akun `@rencan.local` yang didokumentasikan. Password `rencan123` **masih benar** untuk ketiga akun baru (`pw_ok=t`). Login `ceo@fixtures.local` / `rencan123` berhasil, masuk Home sebagai CEO.
- **Dugaan penyebab**: reseed non-destruktif sekitar Sprint 9 (commit "grant view_all_workspace ke fixture staff", lihat entry Sprint 9 di atas) kemungkinan mengganti seed manual-QA lama (`@rencan.local`, 6 role) dengan fixture set lebih kecil (`@fixtures.local`, 3 akun) yang dirancang untuk skenario "DCR" (diduga DB Contract Regression — belum diverifikasi ke source test, ditandai `[?]`). Belum dikonfirmasi commit persis mana yang melakukan reseed ini.
- **Fix wiki**: [[kredensial-login]] ditulis ulang — bagian "Akun aktif saat ini" di atas (3 fixture terverifikasi 2026-07-30), 6-akun lama dipindah ke bagian "Riwayat (stale)" dengan warning eksplisit. `docs/kredensial-login.md` (source repo, bukan wiki) **belum diperbaiki** — masih menyebut 6 akun lama; di luar scope sesi ini (bukan file wiki/).
- **Pelajaran durable**: kredensial dev seed BUKAN stabil selamanya — reseed test-infra (Sprint 9 dkk.) bisa mengganti akun tanpa mengubah dokumentasi manapun. Sebelum pakai kredensial dari wiki/memory untuk aksi yang bisa gagal diam-diam (login), verifikasi read-only ke DB dulu (`pw_ok` query yang sama persis sudah didokumentasikan di halaman ini) — jangan asumsikan "terakhir diverifikasi" masih berlaku.

## [2026-08-02] update | Rantai E2E goal→penyelesaian tugas dijalankan penuh (E01–E27) + 6 akun `@rencan.local` dipulihkan

- **Konteks**: user minta uji skenario "login pakai beberapa akun, buat goal sampai penyelesaian tugas" di `staging.rencanapp.com`. Ditolak karena dua hal mengikat: kredensial staging tak tersedia (placeholder `__STAGING_SEED_PASSWORD__`), dan aturan safety melarang Claude mengetik password apa pun untuk autentikasi — termasuk dev-seed lokal, bahkan saat user eksplisit minta dilanggar. Dialihkan ke Opsi B (susun matriks skenario) + Opsi C (eksekusi di DB lokal).
- **Matriks**: `docs/e2e-scenario-matrix-goal-to-completion.md` — rantai E01–E27 (Goal→KPI Area→Initiative→Action Plan→Task→submit bukti→review approve→nilai masuk KPI→DCR→chat→override skor→tutup periode→D9→repeat instance), 6 checkpoint governance (G01–G06), 5 uji ketahanan (R01–R05).
- **Blocker awal**: DB lokal cuma punya 3 fixture `@fixtures.local` di org terpisah-pisah (lihat [[kredensial-login]] utk detail pemulihan 6 akun `@rencan.local` + gotcha UUID collision `seed_dummy.sql`). Setelah dipulihkan: satu org Nyantuy Group, 6 role saling terhubung dgn rantai card starting-point (4 Goal→Strategy→Initiative→AP dgn PIC benar).
- **Eksekusi**: klik sintetis di harness preview (`mcp__Claude_Browser__computer`) **tidak memicu onPress** Pressable React Native-web (toggle Strategi, tombol Simpan, dll — hanya navigasi URL & isi input yang jalan). Diverifikasi via user pilihan eksplisit (AskUserQuestion): rantai dieksekusi di **lapisan RPC** (impersonasi role via `set_config('request.jwt.claims', …)` dalam transaksi psql, commit setelah validasi rollback), lalu **diverifikasi ulang secara visual** setelah user login manual di browser pane yang sama — kedua lapis cocok 100% (mis. KPI "Akuisisi Customer Baru" 25% di RPC = 25% di UI; deadline DCR 2026-09-15 tampil; skor override 90 + label "Snapshot resmi setelah periode ditutup" tampil persis).
- **Full-tree activation**: guard aktivasi DB terbukti — KPI Area (tabel `strategies`) butuh target+expected_outcome; Initiative (tabel `initiatives`) butuh reason/main_risk/alternative + kontribusi Quarter 100%; Action Plan (tabel `action_plans`) butuh target_result+team; Task (tabel `tasks`) butuh PIC≠Reviewer; Goal butuh ≥1 KPI Area (MBR `blokir_aktivasi`, satu-satunya rule blocking dari 6 rule MBR seed — sisanya `hanya_peringatan`).
- **Temuan bug data lokal (diperbaiki sesi ini)**: `card_completion_rules` punya baris **duplikat org-scoped** untuk `initiative` yang mewajibkan kolom `target_result` — kolom itu **tak ada** di tabel `initiatives` (rename V1.8.3 menggeser field itu ke `action_plans`). `card_completion_rule_for` mengutamakan rule org di atas rule global, jadi aktivasi Initiative Nyantuy **selalu gagal** sebelum diperbaiki. Asal-usul rule yang salah belum ditelusuri (bukan dari `seed_dummy.sql`).
- **Temuan bug UI (dipisah jadi PR #236)**: `workspace-screen.tsx` GoalRow menggerbang tombol "+ Strategi" pakai `can('create_strategy')` — key salah, seharusnya `create_kpi_area` (K4, migration 0010: sengaja BUKAN default management) ATAU jalur `is_goal_pic`. Detail lengkap di [[permission-model]].
- **Temuan desain (dipisah jadi PR #237)**: hub-card lobby Workspace diriset ulang jadi orb capaian sungguhan, menggantikan chip "N/M aktif" yang ternyata redundan dgn stat row. Riwayat 3 putaran keputusan lengkap di [[workspace-hub-orb]].
- **Temuan gap requirement**: override skor SCORE-02 "dua-aktor" di PRD/`manual-testing.md` **sudah usang** — `override_user_score` di DB **single-actor** by design (komentar kode: "D10: single-actor, efektif seketika, anti-self"). Guard yang sungguhan ada: anti-self-override + wajib permission `manage_score_formula` + reason wajib + clamp 0–100. Diverifikasi via 3 skenario RPC (self-override ditolak, permission-kurang ditolak, override CEO→staff berhasil single-actor).
- **Nuansa D9 (visibilitas skor)**: RLS `user_score_results` DAN `ranking_snapshots` **identik** (`own OR manage_score_formula OR view_all_workspace OR is_supervisor_of`) — **tanpa** syarat status-periode di RLS itu sendiri. Aturan "skor peer hanya dari periode tertutup" ditegakkan lewat **gabungan**: klien hanya baca ranking dari periode `closed`, DAN `ranking_snapshots` cuma tercipta saat `close_period_snapshot` dipanggil. Staff biasa tetap tak lihat peer sembarang meski periode tertutup (RLS tetap membatasi ke diri+bawahan).
- **Gate**: full suite 1983 jest hijau (153 suite) pasca kedua PR turunan; matriks sendiri tak punya CI gate (dokumen + eksekusi manual/RPC).
- **Pages created**: `docs/e2e-scenario-matrix-goal-to-completion.md` (bukan halaman wiki — di `docs/`, di luar schema index wiki).
- **Pages updated**: [[kredensial-login]] (ditulis ulang — 9 akun aktif, mekanisme pemulihan), [[permission-model]] (note `create_kpi_area`), [[workspace-hub-orb]] (redesain orb, lihat entry terpisah di bawah), [[index]], `[[log]]` (entry ini).
- **Pelajaran durable**: (1) klaim staleness di wiki bisa **dibalik** oleh kerja sesi berikutnya (6 akun `@rencan.local` yang ditandai "hilang" 2026-07-30 kini pulih) — selalu verifikasi read-only ke sumber sebelum percaya catatan lama, termasuk catatan "sudah basi" itu sendiri. (2) Batasan tooling (klik sintetis harness) bukan alasan berhenti — pivot ke lapisan yang bisa diverifikasi (RPC+RLS sungguhan), lalu tetap tuntaskan verifikasi visual begitu ada jalan (user login manual). (3) Guard aktivasi/permission di DB seringkali **benar** sesuai keputusan mengikat lama (K4 dkk.) — sebelum menyimpulkan "gap permission", telusuri dulu apakah itu keputusan desain sengaja vs bug UI yang salah mengikuti keputusan itu.

## [2026-08-02] update | Fix gerbang "+ Strategi" pakai `create_kpi_area` + jalur `is_goal_pic` (PR #236)

- **Bug**: `GoalRow` (`workspace-screen.tsx:808`) menggerbang tombol "+ Strategi" level Goal dgn `can('create_strategy')` — key yang salah. Policy INSERT `strategies` (tabel KPI Area) sebenarnya butuh `has_permission('create_kpi_area') OR is_goal_pic(goal_id)` (migration 0010, keputusan mengikat K4: *"create_goal/create_kpi_area = CEO/grant, TIDAK default c_level/management; create_strategy tetap default"*). DB sudah benar sejak awal — bug murni di UI, memakai key permission tetangga yang "kelihatan mirip" tapi mengatur aksi berbeda.
- **Efek bug**: dua arah. (a) Manager yang **jadi PIC Goal** — yang server izinkan lewat `is_goal_pic` — TIDAK melihat tombol sama sekali (proxy `create_strategy` tak pernah true utk role manager di `MGR_DEFAULT_KEYS` klien manapun konfigurasinya). (b) Drift risiko: bila daftar default key klien pernah berubah menyertakan `create_strategy`, manager tanpa grant akan sempat lihat tombol lalu server tolak saat submit.
- **Fix**: `canAddKpi = can('create_kpi_area') || (profile?.id != null && goal.pic_id === profile.id)` — mirror policy DB persis, bukan rumus baru.
- **Verifikasi live** (bukan cuma test): probe RPC sbg mgr.sales — `has_permission('create_kpi_area')=false, is_goal_pic('Naikkan Omset Q3')=true`; INSERT KPI Area di Goal miliknya **sukses**, di Goal lain **ditolak RLS**. Lalu user login manual sbg `mgr.sales@rencan.local` (Dewi): tombol "+ Strategi" **tampil** di Goal yang dia PIC, **tersembunyi** di Goal lain — persis prediksi.
- **Tes**: 3 test regresi baru `WSA-13·0·A/B/C` (proxy salah → sembunyi; grant eksplisit → tampil; jalur PIC Goal tanpa grant → tampil) — paralel pola `WSA-13` yang sudah ada utk "+ Inisiatif". 135/135 workspace test lulus, type-check bersih.
- **Gate**: PR [#236](https://github.com/nawawi489/RencanApp/pull/236) → base `fix/canaddkpi-create-kpi-area` (branch berdiri sendiri, di-rebase ke `origin/staging` sebelum push — `223f844` ternyata sudah ada di remote, jadi PR akhirnya 1 commit bersih).
- **Pages updated**: [[permission-model]] (note baru, detail lengkap ada di sana — entry log ini sengaja ringkas).
- **Pelajaran durable**: kalau UI menggerbang aksi via `can(key)`, cocokkan `key`-nya ke policy RLS/RPC yang SEBENARNYA menegakkan aksi itu (baca migration SQL-nya), jangan asumsikan dari nama yang "satu level" dalam hierarki card (Goal/KPI Area/Strategy/Initiative sering ganti nama antar dokumen V1.82→V1.83, gampang salah pasang).

## [2026-08-02] update | Hub-card lobby Workspace: orb capaian sungguhan, satu bentuk dua label (PR #237)

- Redesain lengkap indikator ruang Performance & Development di lobby tab Workspace. Riwayat 3 putaran, opsi yang ditolak beserta alasan, konvensi rollup yang diikuti (bukan dikarang ulang), dan catatan ekspektasi — **semua didokumentasikan penuh di [[workspace-hub-orb]]**, entry log ini sengaja jadi pointer ringkas saja (hindari duplikasi dua tempat).
- **Ringkas**: chip "N/M {parent} aktif" (dari `c7627a6`, 2026-07-29) dihapus karena **redundan** — kedua angkanya sudah tampil di stat row tepat di bawahnya. Diganti `HubProgressOrb`: satu komponen, satu bentuk ring, dipakai KEDUA ruang — label "Capaian" (Performance, ada Goal terukur vs target) atau "Progress" (Development, status-rollup, sengaja **sama bentuknya** dgn Performance karena tree pun begitu — `treeOrbLabel`). Sempat salah rancang bentuk berbeda per ruang (ring vs bar) sebelum dikoreksi oleh pertanyaan user yang menunjuk inkonsistensi vs tree.
- Lobby kini di-scope ke tahun fokus (`usePerformanceItems`/`useDevelopmentItems`, sama dgn pane) — membalik keputusan lama "ringkasan lintas-waktu". Efek samping positif terverifikasi: id sama dgn pane → React Query cache-hit (1 request `workspace_card_progress`, bukan 2, saat lobby+pane sama-sama mount).
- **Gate**: 1983/1983 jest lulus, type-check bersih, lint 0 error. PR [#237](https://github.com/nawawi489/RencanApp/pull/237) → base `fix/canaddkpi-create-kpi-area` (PR #236, bertumpuk — merge #236 dulu).
- **Pages updated**: [[workspace-hub-orb]] (ditulis ulang penuh — riwayat 3 putaran + opsi ditolak), [[index]] (one-liner).

## [2026-08-02] query | Deploy Staging hang 27+ jam — bundle web `staging.rencanapp.com` ketinggalan 3 rilis

- **Konteks**: user tanya kenapa hub-card lobby staging beda dgn lokal (orb `67` amber vs chip "3/3 Goal aktif"). Kode `origin/staging` sudah punya fix `c7627a6` (2026-07-29) — bukan gap kode, gap **deploy**.
- **Temuan**: `gh run list --workflow=deploy-staging.yml` — run terbaru (commit observability `33ad922`) berstatus **`waiting` selama 27+ jam**; semua run sebelumnya sejak `c7627a6` **`cancelled`** (workflow pakai `concurrency: cancel-in-progress: true`, tiap push baru membatalkan run sebelumnya sebelum sempat selesai). Bundle web yang ter-serve (`entry-77d73fffe9e76d87f988e90a573ef913.js`) adalah build lama, pra-fix.
- **Tidak ditindaklanjuti** — user eksplisit minta skip ("kita diskusikan ulang dulu mengenai penggunaan orb%"), yang lalu berujung ke redesain PR #237 di atas. Setelah #237 merge & staging berhasil deploy ulang (kapan pun itu terjadi), staging otomatis dapat tampilan orb baru — TIDAK perlu fix terpisah untuk gap visual ini, cukup deploy sukses.
- **Belum diselesaikan**: kenapa deploy hang 27+ jam (EAS worker queue? credential? build error tersembunyi di balik status `waiting`?) belum didiagnosis — di luar scope sesi ini per keputusan user.

## [2026-08-04] update | P1 optimize pass — FlatList virtualisasi daftar anggota + 2 fix perf lain (PR #243)

- **Konteks**: pass optimasi P1 dari audit `$impeccable` `mobile/`. Tiga temuan perf; inti yang diminta dicatat = konversi daftar anggota org ke `FlatList`.
- **F3 (FlatList — inti)**: dua layar me-render SELURUH anggota org sekaligus tanpa virtualisasi/recycling — `settings-permission-users.tsx` (daftar `.map()` di dalam `ScrollView`) & `settings-org-structure.tsx` `ReportingTab` (`people.map` + satu `UserPicker` per baris). Open-time & jank scroll tumbuh linear terhadap headcount. Dikonversi ke **satu `FlatList` yang selalu ter-mount** (template: `people.tsx`), jadi scroll container sendiri — BUKAN nested di `ScrollView` dalam `KeyboardAvoidingView` (hindari warning VirtualizedList-in-ScrollView). Filter `members` di-`useMemo`, `renderItem` `useCallback` stabil. Header (judul + tombol / TabBar) di `ListHeaderComponent` satu list persisten — kalau loading & loaded dipisah jadi dua pohon (`ScrollView`↔`FlatList`), node tombol ter-unmount di tengah interaksi dan handle `press` jadi basi (ketahuan dari test `[P-UI-08]` yang sempat merah, lalu diperbaiki dgn pola satu-FlatList).
- **Query tak berbatas**: `listOrgProfiles` / `…Admin` / `…WithRoles` (`lib/cards.ts`) tadinya tanpa `.limit()` → ditambah pagination berbasis `.range()` + bound default (`ORG_PROFILES_PAGE_SIZE`). Pemanggil `queryFn` bare dibungkus `() => fn()` supaya context React Query tak masuk ke param opsional baru.
- **F1 (dilipat sekalian)**: `useFocusEffect` di `action-plan/[id]` & `task/[id]` menaruh objek hasil `useQuery` di dep array — `useQuery` mengembalikan `Proxy` baru tiap render → kaskade refetch tiap fokus (`.refetch()` melewati `staleTime`). Diubah ke `}, [])` seperti 8 layar detail saudaranya. Ditambah guard test `no-query-result-in-deps.guard.test.ts` (scan `src`, gagal bila identifier hasil `useQuery`/`useQueries` telanjang di dep array hook).
- **F2 (dilipat sekalian)**: pulse `Skeleton` (`components/ui.tsx`) pakai `useNativeDriver:false` (animasi opacity di thread JS persis saat parse JSON Supabase) → `useNativeDriver:true`, guard `NODE_ENV==='test'` tetap utuh.
- **Add-on**: value context `theme-provider` & `auth-provider` di-`useMemo` (+ `useCallback` `setMode`/`signOut`) — stop re-render consumer app-wide. Memoization `useChatActions` (`use-inbox.ts`) sengaja DITUNDA (tak trivial).
- **Gate**: type-check bersih, lint 0 error, 118 jest hijau lintas semua suite tersentuh + guard. PR [#243](https://github.com/nawawi489/RencanApp/pull/243) → base `staging` (11 file, scope terjaga; tak menyentuh `screen.tsx` / `lib/alert.ts` / modal KAV / `app.json` milik pass P0 in-flight).
- **Gotcha durable**: worktree pakai `node_modules` junction dari `main` (292 commit di belakang staging) yang KURANG `@react-native-community/netinfo` + `expo-updates` → tsc/jest gagal di baseline (file tak tersentuh). Fix tanpa churn: install ke dir scratch, salin HANYA folder package yang hilang (7 disalin, 33 dilewati). JANGAN `npm install` langsung di worktree (rekonsiliasi seluruh pohon shared main).

## [2026-08-05] update | Audit `$impeccable mobile/` — remediasi lengkap 11→19/20 (PR #242–#252)

- **Konteks**: audit teknis 5-dimensi (Accessibility, Performance, Theming, Platform Conformance, Adaptivity) atas `mobile/` lewat skill `$impeccable`, dijalankan lawan **`origin/staging`** — BUKAN `main` (tertinggal 292 commit, terakhir 2026-07-18). Audit pertama sempat jalan di working tree `main` dan menghasilkan **~40% temuan hantu** yang sudah lama diperbaiki di staging (mis. P0 `TextInput` dari bare `react-native`, korupsi env `EXPO_PUBLIactionPlanP_ENV`, token `brand-light` undefined) — dikoreksi dengan re-verifikasi seluruh dimensi lawan worktree read-only `origin/staging`. Lihat gotcha baseline di memory `verify-baseline-origin-staging`.
- **Skor awal 11/20 (Acceptable) → akhir 19/20 (Excellent)** setelah 11 PR. Per dimensi (awal→akhir): A11y 3→4, Perf 3→4, Theming 3→4, **Platform Conformance 1→4**, **Adaptivity 1→3**.
- **Akar sistemik (ditutup TUNTAS + dipagari guard)**: kelas bug prop React Native yang **diam-diam no-op di `react-native-web`** sementara `jest-expo` preset native lapor hijau — 3 agen dimensi berbeda konvergen ke akar yang sama. 4 instans, semua kini tertutup: `Alert.alert` (#242), `KeyboardAvoidingView` di luar `<Modal>` (#242), `hitSlop` sbg touch-target (#250), `accessibilityViewIsModal` di `<View>` (#252). Tiga punya **guard test statik permanen** (`showAlert` lint, `no-hitslop-touch-target.guard`, guard emoji). Detail kelas + gotcha smoke-check di memory `rnw-inert-props-jest-blind-class`.

**Pass per PR (dijalankan tiap satu di sesi terpisah, branch dari `origin/staging`, PR `--base staging` eksplisit):**

- **P0 harden ([#242](https://github.com/nawawi489/RencanApp/pull/242))**: safe-area insets app-wide (kontrak di `screen.tsx` + primitive `bottom-sheet` bersama), 6 konfirmasi destruktif mati di web → seam `showAlert` (`window.confirm` di web supaya callback jalan), `predictiveBackGestureEnabled: true`, 2 `KeyboardAvoidingView` dipindah KE DALAM `<Modal>`-nya.
- **P1 optimize ([#243](https://github.com/nawawi489/RencanApp/pull/243))**: dicatat penuh di entry terpisah di atas (FlatList virtualisasi + kaskade focus-refetch + skeleton thread-JS).
- **P1 adapt ([#245](https://github.com/nawawi489/RencanApp/pull/245))**: sumber kebenaran lebar + `max-w-[720px] self-center` di `screen.tsx` (dulu 0 logika responsif di 61 layar → kolom telepon full-bleed di 1440px web/iPad), grid kolom width-derived, tab bar → nav rail di lebar expanded, KAV form diangkat ke `screen.tsx`, `Button` dapat flex contract.
- **P2 document ([#246](https://github.com/nawawi489/RencanApp/pull/246))**: rekonsiliasi 4 kontradiksi-diri `DESIGN.md` (baris placeholder terbalik vs kode, §2 masih sanksi `useColorScheme()` yang §12 larang, mapping radius §5 salah, konflik glyph §7↔§10). Keputusan owner: state seleksi = Ionicons `checkmark-circle`/`ellipse-outline` (penuhi §4 sinyal-non-warna DAN §10 mandat Ionicons).
- **P1 animate ([#247](https://github.com/nawawi489/RencanApp/pull/247))**: seam `useReduceMotion()` → `Skeleton` + modal hand-rolled honor setelan OS Reduce Motion (dulu 0 query `AccessibilityInfo`).
- **P1 colorize ([#248](https://github.com/nawawi489/RencanApp/pull/248))**: ~45 emoji/glyph ikon (`🗑️ ▾ ⚠ ✓ ○ 💬` dll.) → Ionicons (import per-family, bukan barrel; warna theme-aware via `useThemedIcon`) + guard emoji.
- **P2/P3 layout+polish ([#249](https://github.com/nawawi489/RencanApp/pull/249))**: kontras (`text-red-500`→`red-700` ×12 + label "(wajib)", `amber-600`→`amber-700`, `ErrorState`, `text-brand` 3.43:1), regresi ikon statis `settings-account`, container fixed-height→`minHeight` (survival Dynamic Type), angka orb kompak 9→11px, sel `MonthDaysPicker` 40→44px (gagal di native juga), straggler touch-target 42→44.
- **harden-2 ([#250](https://github.com/nawawi489/RencanApp/pull/250))**: `hitSlop` (28 situs, no-op di web) → kotak 44px NYATA lewat helper `touchTarget` (inner visual view + Pressable transparan + margin negatif → hit area nyata tanpa membesarkan chrome padat tree) + guard; `RowActionsMenu` → `ActionSheetIOS` native (iOS); metafora modal Cancel/Done + system back dipulihkan (workaround web quirk di-`Platform.OS`-guard, tak lagi bocor ke native).
- **P2 cleanup ([#252](https://github.com/nawawi489/RencanApp/pull/252))**: 3 overlay permission `<View absolute inset-0>` → `<Modal>` nyata (**tutup instans terakhir kelas inert-prop**), ~42 hex inline Workspace → token bernama (refactor visual-identical, output byte-identik light+dark), archive N+1 (`useQueries` ~N request) → satu query batch `.in()` (`getArchiveMetadataBatch`), memoize value context `theme`/`auth` + `useChatActions`.

- **Sisa terbuka (1 item, keputusan produk owner — BUKAN defect)**: kunci orientasi iPad di `app.json` (`orientation: portrait`, tanpa `ios.requireFullScreen`) — satu-satunya penahan Adaptivity di 3. Untuk app utilitas-eksekusi, portrait-lock adalah pilihan sah, bukan kegagalan. Owner memilih **berhenti di sini** (2026-08-05): biarkan portrait apa adanya, revisit hanya bila sinyal pemakaian iPad/tablet muncul. `app.json` sengaja TAK disentuh sepanjang remediasi.
- **Pelajaran durable**: (1) working tree `main` bisa jauh tertinggal `origin/staging` — task baca-kode apa pun (audit/review/spec) WAJIB target staging; buat worktree read-only bila `git rev-list --left-right --count main...origin/staging` ≠ 0. (2) Kelas prop-inert RNW butuh **guard statik**, bukan cuma fix per-instans — "hijau jest + merged" TIDAK membuktikan perilaku web (preset native meniru RN, bukan RNW). (3) Orkestrasi: tiap pass = satu sesi terpisah dengan handoff self-contained (baseline discipline + file:line terverifikasi + template in-repo), di-serialize untuk hindari konflik file lintas-pass.
- **Pages updated**: `[[log]]` (entry ini). Index tak berubah — tak ada halaman wiki baru (audit + PR sudah jadi sumber kebenaran; DESIGN.md diperbarui via #246).
