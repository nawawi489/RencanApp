...
## [2026-07-21] update | Verifikasi reboot: atribusi autostart Docker + mode kegagalan socket yatim

- **Uji atribusi (kontrol negatif):** `rencanapp-docker-autostart.vbs` disingkirkan sementara, mesin di-restart. Pada boot+4,4 menit Docker Desktop **nol proses** — tidak pernah diluncurkan, bukan "hidup tapi gagal". Entri HKCU Run `Docker Desktop` ada tapi tidak efektif (`AutoStart=False`). Kontrol positif: Chrome dari entri Run yang sama start di boot+21 detik. Kesimpulan: skrip itu load-bearing.
- **Keepalive runner ikut terverifikasi** pada reboot yang sama — runner `online`, service `active`, tanpa intervensi. Catatan "belum diuji melewati reboot" yang menggantung sejak 2026-07-20 kini tertutup.
- **Mode kegagalan baru yang ditemukan justru saat uji ini:** reboot mendadak meninggalkan socket AF_UNIX yatim (`Docker\run\dockerInference`, lalu `docker-secrets-engine\engine.sock`) yang Docker sendiri tak bisa hapus → aplikasi menutup diri saat start dengan dialog yang hanya menawarkan Quit / Reset to factory defaults.
- **Yang tidak berhasil:** `File.Delete`, `del /f`, path mentah `\?\` — semuanya "The file cannot be accessed by the system". **Yang berhasil:** menyingkirkan folder induknya (`Directory.Move`). Tiap start gagal meninggalkan bangkai baru di lokasi berbeda, jadi kedua folder harus dibersihkan sekaligus.
