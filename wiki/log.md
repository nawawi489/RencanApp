...
## [2026-07-22] update | BL-09(b) selesai — baris arsip bisa dibuka; BL-09 tertutup
...
- **A11y.** `SectionCard` dapat prop `accessibilityLabel` opsional. Kartu pressable sebelumnya hanya punya `accessibilityRole="button"` tanpa label — DESIGN §4.4 mewajibkan keduanya, dan tanpa label screen reader cuma membaca gabungan teks anak sehingga tujuan tap tidak jelas. Baris arsip memakai `Buka detail <nama>`.
- **Utang a11y yang sengaja dibiarkan.** `Pressable` RN default `accessible={true}`, jadi kartu pressable mengumpulkan anaknya jadi satu elemen a11y dan tombol "Pulihkan ke Draft" bersarang tak lagi fokus terpisah di iOS. Ini pola yang **sudah** berlaku (`InstanceRow` di `task/[id].tsx:246` persis begitu); memperbaikinya berarti mengubah kontrak `SectionCard` untuk semua pemakainya — di luar cakupan, dicatat di PR #144 supaya tidak hilang.
- PR: #144 (nol migrasi).
- Files updated: `mobile/src/app/(app)/settings-archive.tsx`, `mobile/src/components/ui.tsx`, `mobile/src/app/(app)/__tests__/fase8-settings-screens.test.tsx`.
- Pages updated: [[feature-gap-backlog]] (baris BL-09 → DONE + §2 triage + catatan penutup di callout "BL-09 bukan satu bug").
- **Verifikasi:** `npm test` **1628 passed / 131 suite**; `npm run type-check` bersih; `npm run lint` 0 error. Tes baru: `[F8-UI-29]` tipe terpetakan → `push('/action-plan/a1')` (sekalian mengunci bahwa `action_plan` memetakan ke segmen ber-tanda-hubung), `[F8-UI-30]` tipe tak terpetakan → tak ada kontrol "Buka detail", `push` tak pernah dipanggil.
