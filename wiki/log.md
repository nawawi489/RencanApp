...
- **Follow-up belum dieksekusi** (4 temuan cleanup minor dari review, tidak correctness-blocking): `strategy/new.tsx` empty-state + button styling seharusnya reuse `EmptyState`/`Button` component alih-alih hand-rolled; migrasi 0059 sanity DO-block bisa disederhanakan (redundant COUNT scan); contract test TEST 1 punya satu EXISTS check yang sudah logically-implied oleh count check di atasnya.
