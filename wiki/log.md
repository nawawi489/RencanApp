...
- **Aman saat server belum ter-deploy.** Edge Function tidak ikut ter-deploy oleh merge. Server lama tidak mengirim `warning` → `readWarning` mengembalikan `null` → perilaku client persis seperti sekarang. Jadi urutan merge-lalu-deploy tidak punya jendela rusak.
- **BELUM LIVE.** Butuh `supabase link` + `supabase functions deploy create-user`; tidak dijalankan di sesi ini (tanpa kredensial link). Sampai itu dilakukan, tiga lubang senyap masih terbuka di produksi meski kodenya sudah mendarat.
