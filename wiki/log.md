...
## [2026-07-23] update | BL-16a selesai — form Posisi/Tim tidak lagi membuang field tautan
...
- Files updated: `mobile/src/app/(app)/settings-org-structure.tsx`, `DESIGN.md` (§7 token `OptionPicker`), [[feature-gap-backlog]] (BL-16a..d baru)
...
## [2026-07-23] update | BL-16b selesai — Departemen bisa dinonaktifkan, anggota Tim bisa dikelola (migrasi 0089)

- Files created: `supabase/migrations/0089_org_structure_write_paths.sql`, `supabase/tests/0089_org_structure_write_paths_contract.sql`, `mobile/src/app/(app)/settings-team/[id].tsx`, `mobile/src/app/(app)/__tests__/settings-team-members.test.tsx`
...
- **Nonaktifkan Departemen TIDAK meng-cascade** ke Posisi/Tim yang menautnya. Ini keputusan, bukan kelalaian: tautan lama justru ALASAN nonaktif dipilih ketimbang delete, jadi "merapikan" dengan `set null` menghancurkan riwayat yang mau diselamatkan. Dikunci `0089-DB-3` dan **diverifikasi merah** dengan varian yang men-`set null` — `position_unlinked` menyala tepat.
...
- **`remove_team_member` memvalidasi keberadaan Tim terpisah dari hasil delete.** Tanpa itu, tim milik org lain dan "orang ini memang bukan anggota" sama-sama menghasilkan 0 baris, dan pesan "bukan anggota" akan mengonfirmasi keberadaan tim lintas-org. Dikunci `0089-DB-6` sebagai uji **anti-oracle** — diverifikasi merah dengan varian tanpa guard: pesannya bocor jadi "Orang ini bukan anggota Tim tersebut".
...
- **Regen `database.types.ts` dari DB LOKAL, bukan staging** — staging belum punya 0089. Diff terverifikasi tepat 2 fungsi baru, nol perubahan lain, dan CRLF dipertahankan.
