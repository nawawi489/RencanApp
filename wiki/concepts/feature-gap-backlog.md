...
| **BL-19c** | Profile / Goal / Org | ~~**Jalur tulis belum ada sama sekali.** (1) `profiles`: nol write di seluruh `src/` meski RLS `profiles_update_self` siap sejak 0001; (2) `updateGoal` tidak ada; (3) `organizations` read-only~~ ✅ **SELESAI 2026-07-23 — migrasi 0093.** Klaim (1) **terkoreksi saat implementasi**: `profiles_update_self` bukan "siap dipakai" melainkan **lubang eskalasi hak akses yang bisa dieksploitasi** — RLS tidak punya granularitas kolom, jadi policy self-update mengizinkan pemilik baris menyetel `role_template_id` miliknya jadi CEO (juga `organization_id` = pindah tenant, `is_active` = hidupkan akun yang dimatikan admin). Ketiganya diverifikasi berjalan di DB dev. Perbaikannya bukan mempersempit policy melainkan **menghapusnya + mencabut GRANT tulis**; batas kolom pindah ke badan RPC (`update_own_profile` = `full_name` saja). Keputusan owner: Goal aktif → nama/deskripsi/PIC boleh, **periode + target terkunci** (dasar perhitungan skor), ditolak eksplisit bukan diabaikan. Zona waktu org divalidasi terhadap katalog Postgres, bukan daftar hardcoded. Tiga layar: `/settings-profile`, `/settings-organization`, `/goal/edit/[id]`. Dikunci `0093-DB-1..11` (DB-3 diverifikasi merah) + `[C-01..C-08]` + `[D-01..D-06]` | M | ✅ DONE |
| **BL-19d** | Role template / Reporting line | ~~Role Template kustom bisa dibuat tapi tak bisa di-assign; Reporting line §34.3 absen di semua layer~~ ✅ **SELESAI 2026-07-23 — migrasi 0094. PRD §34.3 LENGKAP 6/6.** **(1) Template assignable**: `create-user` menerima `role_template_id` opsional. Invarian keamanan — **level efektif diturunkan dari BARIS TEMPLATE DI DB, tidak pernah dari `role_level` kiriman klien**; kalau level ikut dipercaya, penyerang cukup mengirim template C-Level bersama `role_level: staff` untuk melewati guard eskalasi, dan fitur ini justru jadi vektor eskalasi. Template diambil dengan filter organisasi (tanpa itu, id template org lain diterima). Tanpa `role_template_id`, perilaku lama dipertahankan persis (baris seeded tertua per level). **(2) Reporting line**: kolom `profiles.manager_id` + RPC `set_reporting_line`. **Cakupan V1 DESKRIPTIF, bukan otorisasi** — dan itu keputusan berdasar temuan, bukan kehati-hatian abstrak: `user_permissions.scope` ternyata **tidak dibaca satu pun policy/RPC** (klien hanya menampilkannya), jadi tidak ada mesin scope yang menunggu disambung; menyambungnya sekarang = membangun mesin otorisasi baru sekaligus. Penjaga siklus ditulis sebagai **loop berbatas kedalaman, bukan CTE rekursif** (CTE rekursif atas data yang sudah bersiklus menggantung sebelum sempat melapor). FK `ON DELETE SET NULL`, bukan CASCADE. Dikunci `0094-DB-1..8` + `[D-01..D-08]`; DB-3 diverifikasi merah dengan penjaga satu-tingkat: siklus a→b→c→a lolos | L | ✅ DONE |

---

## 2. Triage (pasca-verifikasi)

> [!note] Sisa pekerjaan per 2026-07-23: **dua item** — BL-18, BL-19d
> Seluruh BL-01..BL-15 tertutup; PRD §38 lengkap 14/14 lewat migrasi 0085-0088. ~~BL-16~~ ✅ **selesai 2026-07-23 (PR #165, migrasi 0089)**; ~~BL-17~~ ✅ **selesai 2026-07-23 (PR #166, nol migrasi)**. Dari trio sisa-utang-BL-10 itu yang tersisa **BL-18**; ketiganya lahir dari menyisir sisa utang BL-10 secara sengaja — bukan dari audit baru. **BL-19a..d** datang dari jalur berbeda (audit jalur tulis struktur organisasi, PR #169): a/b/c sudah tutup, sisa **d** menyusul BL-18 di antrean. BL-19c naik prioritas di tengah jalan karena ternyata membawa **lubang eskalasi hak akses yang bisa dieksploitasi**, bukan sekadar fitur yang belum ada.
>
