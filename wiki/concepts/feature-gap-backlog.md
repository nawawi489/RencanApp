...
| **BL-10b** | Search | Scope **People** (migrasi 0086). Gate paling sederhana (`profiles_select_same_org`), tanpa permission gate. Menutup utang FR-8.5.3 Fase 8 yang shipped 7 dari 8 entity type tanpa catatan keputusan — **perbaikan FR yang meleset, bukan fitur baru**. Hasil wajib de-scored (nama + jabatan saja) | S | ⏳ berikutnya |
| **BL-10c** | Search | Scope turunan: `task_instance`, `comment`, `evidence` (migrasi 0087). Mewarisi `can_access_task`/`can_access_action_plan` yang sudah confidential-aware pasca-0077. **Diblokir BL10-OQ-03** (bukti berstatus draft) | M | 🔒 blocked |
| **BL-10d** | Search | Scope audit: `activity_log`, `governance_violation` (migrasi 0088). Sengaja **paling akhir** — dua keputusan owner belum final (BL10-OQ-01/02), definisi match belum ada (OQ-05), dan tabelnya append-only sehingga salah tulis tak bisa dikoreksi. Catatan: keduanya **bukan** admin-only, policy ber-OR dengan self-row | M | 🔒 blocked |
