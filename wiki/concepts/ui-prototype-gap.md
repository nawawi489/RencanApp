...
| **UI-G-017** (alias **BL-08**) | **Aksi review ke-3 "Catatan" hilang** — PRD §24.3 mendefinisikan 3 aksi (Setujui, Minta Revisi, Catatan); panel review hanya mengimplementasi 2 | `components/review-submission-panel.tsx` (dipakai `task/[id].tsx` + `task/instance/[id].tsx`) | Aksi "Catatan" NON-TERMINAL: kirim umpan balik reviewer tanpa mengubah status submission ✅ **IMPLEMENTED 2026-07-20** — form catatan ketiga di `review-submission-panel.tsx` (aksi terminal disembunyikan saat menulis; form hanya reset saat kirim berhasil sehingga teks tidak hilang bila gagal) + `lib/inbox.ts::postReviewNote` (resolve room Rencana Aksi → `send_chat_message` ber-konteks Tugas). Submission tetap `pending`, `reviews` tidak disentuh. Jest: 8 test komponen + 10 test data layer. **Sisa ter-defer:** entri Activity Log — lihat §2.2. |
...
## 2.2 Ter-defer: Activity Log untuk aksi "Catatan" (UI-G-017 / BL-08)

Separuh dari UI-G-017 **belum** dikerjakan: aksi "Catatan" tidak menulis entri `activity_logs`.
...
Terbuka juga (belum diputuskan): siapa yang boleh meninggalkan Catatan. Implementasi saat ini mengikuti panel review — hanya reviewer yang ditunjuk, karena panel hanya dirender untuk reviewer.

**Batasan diketahui — Tugas jalur Development belum bisa menerima Catatan.** Room chat menempel pada Rencana Aksi (`chat_rooms.action_plan_id`), sedangkan `tasks.action_plan_id` **nullable**: Tugas yang induknya Problem Statement (jalur Development, parent mutually-exclusive) tidak punya Rencana Aksi sehingga tidak punya room. Aksi Catatan pada Tugas semacam itu ditolak dengan pesan "Tugas ini belum terhubung ke Diskusi Rencana Aksi." Menutup celah ini butuh keputusan produk: beri room sendiri pada Problem Statement, atau nyatakan Catatan memang hanya untuk jalur Performance.
...
| UI-S-W08 | **Dim "periode lewat" bertumpuk multiplikatif**: wrapper `opacity-50` per level bersarang → level-3 past dalam ancestor past dirender 0.125 (gagal kontras AA, DESIGN §4). Temuan design-consultation 2026-07-02. | MINOR ✅ **RESOLVED-BY-DECISION 2026-07-03** — dim dihapus seluruhnya, bukan diperbaiki. Fix awal 2026-07-02 (`PastDim` single-layer + `ancestorPast` threading) **dibatalkan owner decision 2026-07-03**: kartu periode-lewat tidak lagi didim di layer manapun; badge teks "Periode lewat" jadi satu-satunya sinyal (DESIGN §4 terpenuhi — teks, bukan warna/opacity). Tes [W08·1..2] kini menjaga arah sebaliknya (`countOpacityHalf === 0`). Lihat `log.md` [2026-07-03]. Menyimpang dari PRD §44 AC-9 — lihat catatan di sana. |
