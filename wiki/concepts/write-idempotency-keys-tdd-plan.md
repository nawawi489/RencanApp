...
6. **Migration number** — verify highest vs `origin/staging` via `git ls-tree` (= 0099 → next 0100). Wrong number → collision / non-contiguous gap.
...
- [[write-idempotency-keys]] — the spec (corrected: `author_id`, plain index, next migration 0100).
