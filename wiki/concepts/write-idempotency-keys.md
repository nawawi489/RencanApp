...
1. One migration (**`0103`**) adds columns + a plain partial unique index (see §Design — **not** `CONCURRENTLY`, since the new nullable column matches zero existing rows so the index builds instantly inside the migration txn) for the five direct-insert tables and `chat_messages`. Number lesson: `origin/staging @ adca441` already has `0100`/`0101`/`0102`; an earlier check wrongly read the highest as `0099` because the grep pattern `00[0-9]{2}` cannot match `0100`+. `supabase db reset` (replaying `0001→0103`) is what actually caught the collision — trust a full replay over a name-scan per [[../MEMORY|verify-baseline-origin-staging]].
