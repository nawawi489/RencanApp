...
> [!warning] Men-supersede keputusan owner 2026-07-03 poin 1 (Initiative & Action Plan)
> Keputusan owner terkunci di [[workspace-progress-orb-tdd-plan]] poin 1 (2026-07-03) menetapkan **rumus induk = `% anak LANGSUNG berstatus done` (non-rekursif)** untuk SEMUA level induk. 0118 (Opsi B) **membatalkan keputusan itu khusus untuk Initiative & Action Plan**: kedua level kini rata-rata **rekursif** progress anak (leaf Tugas → AP → Initiative), menutup gap yang justru dicatat sebagai "Keputusan terbuka" di halaman itu (Initiative tampil 0% walau satu-satunya AP anak 80%). **Semua level lain tetap** memakai keputusan 2026-07-03 (count-done / attainment). Klien: `ratioDoneOfChildren` diturunkan dari "rumus induk" menjadi **fallback saja** saat RPC `null` (loading/error) di header detail Initiative/AP.
