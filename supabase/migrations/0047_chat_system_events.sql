...
--
-- Resolusi: action_plan → initiative → chat_room.
-- Room 0 baris → RETURN diam-diam (US-8: Initiative belum aktif / Tugas yatim).
...
begin
  ...
  select initiative_id into v_initiative
  from public.action_plans where id = p_action_plan;
  if v_initiative is null then return; end if;

  select id, organization_id into v_room_id, v_org
  from public.chat_rooms where initiative_id = v_initiative;
...
-- ============================================================ 3. TRIGGER: action_plans status change
--
-- Mapping:
...
drop trigger if exists tg_action_plan_system_event on public.action_plans;
create trigger tg_action_plan_system_event
  after update of status on public.action_plans
  for each row
  when (OLD.status is distinct from NEW.status)
  execute function public.tg_action_plan_system_event();

-- ============================================================ 4. TRIGGER: action_plan_instances status change
...
begin
  ...
  if v_type is not null then
    perform public.emit_chat_system_event(NEW.action_plan_id, auth.uid(), v_type);
  end if;
...
drop trigger if exists tg_instance_system_event on public.action_plan_instances;
create trigger tg_instance_system_event
  after update of status on public.action_plan_instances
  for each row
  when (OLD.status is distinct from NEW.status)
...
begin
  if auth.uid() is null then return NEW; end if;
  if NEW.entity_type <> 'action_plan' then return NEW; end if;
