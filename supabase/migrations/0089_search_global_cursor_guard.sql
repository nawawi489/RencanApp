-- BL-16 — perketat guard bentuk-request FR-19: cursor SEPARUH ditolak.
--
-- Sebelum migrasi ini, guard hanya menuntut `p_scopes` berisi tepat satu scope. Ia tidak
-- menuntut kedua bagian cursor terisi, sehingga `p_cursor_ts` terisi + `p_cursor_id` NULL
-- lolos ke seluruh cabang, yang memfilter dengan `(created_at, id) < (p_cursor_ts, p_cursor_id)`.
--
-- Postgres membandingkan tuple secara SHORT-CIRCUIT, jadi selama `created_at < p_cursor_ts`
-- tegas, `id` tak pernah disentuh dan hasilnya benar. Bug muncul HANYA pada kasus TIE —
-- `created_at` persis sama dengan cursor, satu-satunya kondisi `id` ada untuk memecahkannya.
-- Di sana perbandingan jatuh ke `id < NULL` → NULL → seluruh baris tie gugur TANPA error.
--
-- Cakupannya sempit dan klien tidak pernah menghasilkannya (`useSearchScopePage` selalu
-- mengirim kedua bagian dari kolom non-null) — ia hanya terjangkau lewat panggilan API
-- tangan. Yang membuatnya layak diperbaiki adalah GEJALANYA: kehilangan data tanpa error,
-- jadi tidak akan ada yang melaporkannya. Bentuk kebalikannya (`p_cursor_id` terisi,
-- `p_cursor_ts` NULL) sama-sama ditolak: hari ini ia diabaikan diam-diam oleh gate
-- `p_cursor_ts is null` di tiap cabang, sehingga pemanggil menerima halaman pertama lagi
-- sambil mengira ia meminta halaman berikutnya.
--
-- Dikunci `supabase/tests/0089_search_global_cursor_guard_contract.sql` (DB-96..DB-101).
--
-- `create or replace`, BUKAN `drop`+`create` — `drop ... cascade` mereset ACL fungsi ke
-- PUBLIC EXECUTE dan membatalkan revoke 0085. Karena itu badan fungsi ditulis ulang utuh,
-- disalin VERBATIM dari 0088 (versi terlengkap, 14 scope); satu-satunya perubahan ada di
-- blok guard di bawah.
--
...
begin
  ...
  -- BL-16 — cursor sah hanya bila KEDUA bagian terisi, atau keduanya NULL.
  -- Cursor separuh bukan "cursor yang kurang lengkap" melainkan permintaan yang tidak
  -- punya arti: pasangan (ts, id) adalah SATU posisi dalam urutan `created_at desc, id desc`,
  -- dan separuhnya tidak menunjuk posisi mana pun. Ditolak di sini, bukan di tiap cabang,
  -- supaya cabang ke-15 yang ditambahkan nanti otomatis ikut terlindungi.
