...
alter table public.chat_messages
  add constraint chat_messages_context_entity_type_enum
    check (context_entity_type is null or context_entity_type in ('action_plan'));
...
begin
  ...
  -- Validasi konteks Action Plan (FR-RC-3).
  if p_context_action_plan is not null then
    select 'action_plan', a.id, a.name
      into v_context_type, v_context_id, v_context_label
    from public.action_plans a
    where a.id = p_context_action_plan;
    if v_context_id is null then
      raise exception 'Action Plan konteks tidak ditemukan.';
    end if;
    -- Anti-spoof: AP harus milik Initiative yang sama dengan room.
    if not exists (
      select 1 from public.action_plans a
      where a.id = p_context_action_plan
        and a.initiative_id = v_room.initiative_id
    ) then
      raise exception 'Action Plan tidak berada dalam Initiative room ini.';
    end if;
  end if;
