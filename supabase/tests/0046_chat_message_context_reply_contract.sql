...
begin
  ...
  insert into public.profiles (id, organization_id, full_name, email, role)
    values (v_user1, v_org, 'Alice', 'alice-0046c@test.local', 'staff'),
           (v_user2, v_org, 'Bob',   'bob-0046c@test.local',   'staff');
  ...
  if v_row.context_entity_type <> 'action_plan' then
...
begin
  ...
  v_user1 := gen_random_uuid();
  insert into public.profiles (id, organization_id, full_name, email, role)
    values (v_user1, v_org, 'Carol', 'carol-0046d@test.local', 'staff');

  -- Init 1 + room
  insert into public.initiatives (id, organization_id, name, status, created_by)
    values (gen_random_uuid(), v_org, 'Init-1', 'active', v_user1)
    returning id into v_init1;
  select id into v_room1 from public.chat_rooms where initiative_id = v_init1;
  ...
  insert into public.action_plans (id, organization_id, initiative_id, name, pic_id, reviewer_id, status, created_by)
    values (gen_random_uuid(), v_org, v_init2, 'AP-Init2', v_user1, v_user1, 'assigned', v_user1)
  ...
  exception when others then
    if sqlerrm not like '%Initiative%' then
      delete from public.organizations where id = v_org;
...
begin
  ...
  v_user1 := gen_random_uuid();
  insert into public.profiles (id, organization_id, full_name, email, role)
    values (v_user1, v_org, 'Dave', 'dave-0046e@test.local', 'staff');
  ...
  insert into public.initiatives (id, organization_id, name, status, created_by)
    ...
    returning id into v_init2;
  select id into v_room2 from public.chat_rooms where initiative_id = v_init2;
...
begin
  ...
  v_user1 := gen_random_uuid();
  insert into public.profiles (id, organization_id, full_name, email, role)
    values (v_user1, v_org, 'Eve', 'eve-0046f@test.local', 'staff');
...
begin
  ...
  v_user2 := gen_random_uuid();
  insert into public.profiles (id, organization_id, full_name, email, role)
    values (v_user1, v_org, 'Fay', 'fay-0046g@test.local', 'staff'),
           (v_user2, v_org, 'Gus', 'gus-0046g@test.local', 'staff');
