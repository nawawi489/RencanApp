...
begin
  ...
  insert into public.task_submissions (id, task_id, version_number, submitted_by, status, review_status)
    values (gen_random_uuid(), v_ap, 1, v_pic, 'submitted', 'pending')
    returning id into v_sub;
  update public.tasks set current_submission_id = v_sub where id = v_ap;

  -- Act: reviewer approves
  perform set_config('request.jwt.claims', json_build_object('sub', v_reviewer)::text, true);
  perform public.review_task_submission(v_sub, 'approve', null);
  ...
  if v_ev.context_entity_type <> 'task' or v_ev.context_entity_id <> v_ap then
  ...
  delete from public.task_submissions where task_id = v_ap;
  delete from public.tasks where id = v_ap;
  delete from public.action_plans where id = v_init;
...
begin
  ...
  perform set_config('request.jwt.claims', json_build_object('sub', v_reviewer)::text, true);
  perform public.review_task_submission(v_sub, 'reject', 'Perlu perbaikan');
...
begin
  ...
  insert into public.chat_room_members (chat_room_id, member_id)
    values (v_room, v_pic), (v_room, v_reviewer) on conflict do nothing;
  insert into public.tasks (id, organization_id, action_plan_id, name, pic_id, reviewer_id,
  ...
  insert into public.task_submissions (id, task_id, version_number, submitted_by, status)
    values (gen_random_uuid(), v_ap, 1, v_pic, 'draft')
    returning id into v_sub1;
  perform public.submit_task(v_sub1, 'catatan', null, null);

  -- Reviewer rejects
  perform set_config('request.jwt.claims', json_build_object('sub', v_reviewer)::text, true);
  perform public.review_task_submission(v_sub1, 'reject', 'Revisi dulu');
  ...
  perform public.submit_task(v_sub2, 'sudah diperbaiki', null, null);
