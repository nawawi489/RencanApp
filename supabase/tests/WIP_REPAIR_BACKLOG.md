...
- **`0017_permission_settings_contract`** (un-quarantined 2026-07-19) — the
  suspected grant/behaviour drift was confirmed as real security regression, not
  test staleness. Fixed by migration
  `0072_restore_user_permissions_hardening.sql` (PR #105 → staging), which
  re-revokes direct INSERT/UPDATE/DELETE on `public.user_permissions` from
  `authenticated`/`anon` (undone by `0036`'s bulk `GRANT`), restores the
  self-modification guard in `set_user_permission` (dropped by `0041`'s
  rewrite), and restores the invalid-key wording (`'Kunci hak akses tidak
  valid: %'`) that D_key asserts.
